...
export function parseDomainDecomposerContract(
  ...
  const subtasks = obj.subtasks.map((entry, index) => {
    ...
    const intentId = asString(subtask.intentId, `subtasks[${index}].intentId`) || fallbackIntentId;
  ...
  const tasks = obj.tasks.map((entry, index) => {
    ...
    const dependsOn =
      task.dependsOn === undefined
        ? []
        : asStringArray(task.dependsOn, `tasks[${index}].dependsOn`);
...
export function parseToolAnalysisContract(raw: string): ToolAnalysisContract {
  ...
  const riskLevel = obj.riskLevel === undefined ? 'low' : asRiskLevel(obj.riskLevel, 'riskLevel');
  const missingData =
    obj.missingData === undefined ? [] : asStringArray(obj.missingData, 'missingData');
...
export function parseToolArgumentContract(raw: string): ToolArgumentContract {
  ...
  return {
    ...
    ...(asString(obj.rationale, 'rationale')
      ? { rationale: asString(obj.rationale, 'rationale') }
      : {}),
  };
}
