...
function buildDeterministicToolArgs(
  _spec: ToolSpec,
  _input: AnneOrchestratorInput
  ...
  try {
    ...
    for (const task of tasks) {
      ...
      for (const step of steps) {
        ...
        const safetyDecision = evaluateToolSafety(toolSpec, normalizedCall, {
          input,
          classification,
          pipelineId,
          taskId: task.id,
