...
async function createTestUser(): Promise<TestUser> {
  ...
  const { data: user, error } = await supabase
    ...
    .insert({
      ...
      wallet_balance: 0.0, // Initial balance €0
      account_type: 'user',
    })
    .select()
    .single();
...
async function simulateConcurrentTraffic(
  ...
  const operations = Array.from({ length: concurrency }, (_, index) => {
    return addWalletCreditWithRetry(userId, amount, index + 1).then((result) => ({
      index: index + 1,
...
async function runStressTest() {
  ...
  try {
    ...
    const AMOUNT_PER_OPERATION = 10.0;
    ...
    console.log(`\n📊 Final Verification:`);
    console.log(
      `   Expected balance: €${expectedBalance.toFixed(2)} (${successful} operations × €${AMOUNT_PER_OPERATION})`
    );
    ...
    if (successful === CONCURRENCY) {
      ...
      console.log(`   ✅ ${successful}/${CONCURRENCY} operations successful`);
      console.log(
        `   ✅ Final balance: €${finalBalance.toFixed(2)} (matches successful operations)`
      );
