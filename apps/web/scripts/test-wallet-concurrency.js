...
async function createTestUser() {
  ...
  const { data: user, error } = await supabase
    ...
    .insert({
      ...
      wallet_balance: 10.0, // Initial balance
      account_type: 'user',
    })
    .select()
    .single();
...
async function testConcurrentDebit() {
  ...
  try {
    ...
    const checks = {
      ...
      'Final balance = €0.00': Math.abs(finalBalance - 0.0) < 0.01,
...
async function testInsufficientBalance() {
  ...
  try {
    ...
    // Set balance to €5
    await supabase.from('users').update({ wallet_balance: 5.0 }).eq('id', testUser.id);
    ...
    const checks = {
      ...
      'Balance unchanged (€5.00)': Math.abs(finalBalance - 5.0) < 0.01,
    };
...
async function testNoDoubleCredit() {
  ...
  try {
    ...
    // Set balance to €0
    await supabase.from('users').update({ wallet_balance: 0.0 }).eq('id', testUser.id);
    ...
    const checks = {
      'Balance is €100.00 (NOT €200)': Math.abs(finalBalance - 100.0) < 0.01,
      'Balance NOT doubled': finalBalance < 150.0,
      'Transactions sum = balance': Math.abs(finalBalance - transactionsSum) < 0.01,
      'No double credit detected': Math.abs(finalBalance - 100.0) < 0.01,
    };
