...
async function runTest() {
  console.log('🧪 Starting Multi-Account Logic Verification (Dynamic Import Mode)...');

  // Dynamic imports ensure env vars are present before client.ts is evaluated
  const { supabaseAdmin } = await import('@/lib/db/client');
  const { getShippingProvider, getCourierConfigForUser } = await import('@/lib/couriers/factory');
  ...
  const fetchedConfig = await getCourierConfigForUser(user.id, 'spedisci_online', config.id);
  ...
  try {
    const provider = await getShippingProvider(user.id, 'spedisci_online', undefined, config.id);
