...
async function main() {
  ...
  try {
    ...
    console.log(
      `✅ Utente trovato: ${user.id} (${user.account_type}, reseller: ${user.is_reseller})\n`
    );
    ...
    if (!configs || configs.length === 0) {
      ...
      for (const config of configs) {
        ...
        try {
          ...
          if (cacheTestResult.success) {
            const wasCached =
              cacheTestResult.priceListsCreated === 0 &&
        ...
        try {
          ...
          if (incrementalTestResult.success) {
            console.log(
              `   ✅ Sync incrementale OK: ${incrementalTestResult.entriesAdded || 0} entries aggiunte`
            );
          } else {
      ...
      if (finalPriceLists) {
        ...
        for (const pl of finalPriceLists) {
          ...
          const hasValidStructure = metadata.carrier_code || metadata.contract_code || pl.name;
        ...
        console.log(
          `✅ Compatibilità: ${compatibleCount}/${finalPriceLists.length} listini compatibili`
        );
    ...
    if (testPriceList) {
      try {
        ...
        if (configResult.success) {
          ...
          console.log(
            `⚠️ Configurazione non trovata (normale se non ancora creata): ${configResult.error}`
          );
        }
      } catch (error: any) {
        results.push({
