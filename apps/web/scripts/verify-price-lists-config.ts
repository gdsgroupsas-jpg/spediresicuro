...
async function verifyPriceListsConfig() {
  ...
  const { data: glsLists, error: glsError } = await supabaseAdmin
    ...
    .or(
      `metadata->>'carrier_code' ilike '%gls%', metadata->>'contract_code' ilike '%gls%', source_metadata->>'carrier_code' ilike '%gls%', source_metadata->>'contract_code' ilike '%gls%', name ilike '%gls%'`
    )
    .eq('status', 'active')
  ...
  const glsCustomWithMaster =
    glsLists?.filter((l: PriceListConfig) => l.list_type === 'custom' && l.master_list_id).length ||
    0;

  const glsSupplierWithoutMargin =
    glsLists?.filter(
      (l: PriceListConfig) =>
  ...
  const posteCustomWithMaster =
    posteLists?.filter((l: PriceListConfig) => l.list_type === 'custom' && l.master_list_id)
      .length || 0;

  const posteSupplierWithoutMargin =
    posteLists?.filter(
      (l: PriceListConfig) =>
        l.list_type === 'supplier' &&
        !l.master_list_id &&
  ...
  if (glsCustomWithMaster > 0 && posteSupplierWithoutMargin > 0) {
    console.log(`\n✅ SPIEGAZIONE:`);
    console.log(
      `   GLS ha listini CUSTOM con master_list_id → supplierPrice calcolato correttamente`
    );
    console.log(
      `   Poste ha listini SUPPLIER senza margine → supplierPrice = undefined, finalPrice = totalCost`
    );
    console.log(
      `   → Nel route, fallback usa totalCost come supplierPrice → costo = prezzo vendita`
    );
  }
