...
async function main() {
  ...
  const userA = await createSmokeTestUser({
    supabase,
    initialBalance: 50,
    label: 'courier-fail-refund-ok',
  });
  ...
  try {
    ...
    const shipmentsA = await fetchRows({
      supabase,
      table: 'shipments',
    ...
    if (queueA.length !== 0)
      throw new Error(`Expected 0 compensation_queue rows, got ${queueA.length}`);
  ...
  const userB = await createSmokeTestUser({
    supabase,
    initialBalance: 50,
    label: 'courier-fail-refund-fail',
  });
  ...
  try {
    ...
    const shipmentsB = await fetchRows({
      supabase,
      table: 'shipments',
    ...
    if (refundRows.length < 1) {
      throw new Error(
        `Expected REFUND PENDING in compensation_queue, got: ${JSON.stringify(queueB)}`
      );
    }
    ...
    if (!(balanceAfterB < balanceBeforeB)) {
      throw new Error(
        `Expected wallet to be debited (refund failed). Before=${balanceBeforeB} After=${balanceAfterB}`
      );
    }
