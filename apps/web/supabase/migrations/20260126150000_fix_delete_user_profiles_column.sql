-- =====================================================
-- Migration: 20260126150000_fix_delete_user_profiles_column.sql
-- Descrizione: Fix delete_user_complete per gestire user_profiles correttamente
--
-- BUG FIX: La tabella user_profiles usa 'email' come chiave, non 'user_id'
-- =====================================================

-- Prima elimina la funzione esistente
DROP FUNCTION IF EXISTS delete_user_complete(UUID, UUID, TEXT, TEXT, TEXT);

-- Ricrea la funzione con gestione corretta di user_profiles
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_features') THEN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_profiles') THEN
    ...
    ELSE
      ...
      DELETE FROM user_profiles WHERE email = p_target_user_email;
    END IF;
...
'ENTERPRISE-GRADE atomic user deletion function (v3 - dynamic column detection):
1. Soft-deletes all shipments (preserves audit trail)
2. Removes user features (if table exists)
3. Removes user profiles (uses email or user_id based on schema)
