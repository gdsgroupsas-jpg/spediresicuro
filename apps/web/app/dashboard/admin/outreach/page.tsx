...
/**
 * Admin Outreach Dashboard — Sprint S4c
 *
 * Panoramica outreach multi-canale: metriche, enrollment attivi, sequenze.
 * Solo admin/superadmin. Pattern UI identico a admin/leads/page.tsx.
 */

import { useState, useEffect } from 'react';
import {
  getOutreachOverviewAdmin,
  getEnrollmentsAdmin,
  getSequencesAdmin,
...
export default function AdminOutreachPage() {
  ...
  // Carica overview all'avvio
  useEffect(() => {
    setLoading(true);
    getOutreachOverviewAdmin().then((result) => {
  ...
  return (
    <div className="p-6 max-w-[1600px] mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-500 text-sm">Monitoraggio invii, enrollment e sequenze outreach.</p>
      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
              <div className="text-xs text-gray-500 uppercase font-bold">Sequenze Totali</div>
              <div className="text-3xl font-bold text-gray-900 mt-1">{overview.totalSequences}</div>
            </div>
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
              <div className="text-xs text-gray-500 uppercase font-bold">Sequenze Attive</div>
              <div className="text-3xl font-bold text-green-600 mt-1">
                {overview.activeSequences}
              </div>
            </div>
          </div>
        </div>
      ...
      {activeTab === 'sequenze' && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          {sequences.length === 0 ? (
            <div className="p-12 text-center text-gray-400">Nessuna sequenza configurata.</div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left">
                <thead className="bg-gray-50/50 border-b border-gray-200 text-gray-500">
                  ...
                  {sequences.map((seq) => (
                    <tr key={seq.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-5 py-3 text-gray-600 text-xs">
                        {seq.target_statuses?.join(', ') || 'Tutti'}
                      </td>
                      <td className="px-5 py-3 text-gray-400 text-xs">
