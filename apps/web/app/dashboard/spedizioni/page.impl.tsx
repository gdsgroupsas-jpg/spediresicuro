...
import { ImportedBadge, ReturnBadge, StatusBadge } from './_components/shipment-badges';
import {
  DeleteConfirmModal,
  ImportOrdersModal,
  ImportResultModal,
} from './_components/shipment-dialogs';
import { useFilteredShipments } from './hooks/use-filtered-shipments';
...
export default function ListaSpedizioniPage() {
  ...
  // Filtri estratti in hook dedicato per ridurre complessita del container.
  const filteredSpedizioni = useFilteredShipments({
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <ImportOrdersModal
          ...
          onImportComplete={async (successCount, errors) => {
            ...
            // Ricarica immediatamente le spedizioni senza refresh pagina completa.
            try {
              const response = await fetch('/api/spedizioni');
              if (response.ok) {
                const result = await response.json();
        ...
        <ImportResultModal
          result={importResult && !showImportModal ? importResult : null}
          onClose={() => {
            setImportResult(null);
          }}
        />
        ...
        <DeleteConfirmModal
          ...
          shipmentCount={spedizioniToDelete.size}
