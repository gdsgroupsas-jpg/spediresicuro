...
export async function createInvoice(data: CreateInvoiceDTO) {
  ...
  const { error: itemsError } = await supabase.from('invoice_items').insert(itemsToAdd);
...
export async function getInvoiceById(id: string) {
  ...
  const { data, error } = await supabase
    .from('invoices')
    .select(
      '*, items:invoice_items(*), user:users(name, email, company_name, vat_number, address, city, province, zip, country)'
    )
    .eq('id', id)
    .single();
