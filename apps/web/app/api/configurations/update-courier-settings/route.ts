...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!configId || typeof configId !== 'string') {
      return NextResponse.json({ error: 'configId richiesto' }, { status: 400 });
    }
    ...
    if (sanitizedCouriers.length === 0) {
      return NextResponse.json({ error: 'Nessun corriere valido fornito' }, { status: 400 });
    }
    ...
    const isAdmin = actorAccountType === 'admin' || actorAccountType === 'superadmin';
    ...
    if (updateError) {
      ...
      return NextResponse.json({ error: 'Errore salvataggio preferenze' }, { status: 500 });
    }
