 ...
 * 5. Route via ANNE orchestrator runner (same as web chat, mode-aware)
...
import { sendPricingToWhatsApp, sendBookingToWhatsApp } from '@/lib/services/whatsapp-formatter';
...
async function processIncomingMessage(
  ...
  try {
    ...
    // 3. Route via orchestrator mode runner (same as web chat)
    ...
    if (flowResult.pricingOptions && flowResult.pricingOptions.length > 0) {
      await sendPricingToWhatsApp(phone, flowResult.pricingOptions as any);
      return;
    }
 ...
 */
async function lookupUserByPhone(phone: string): Promise<{
  id: string;
  email: string;
  role: AccountType;
  name?: string;
