...
import { runAnneFlowByMode } from '@/lib/agent/v2';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // ===== ANNE ORCHESTRATOR RUNNER =====
    // Runtime governato da mode: shadow | canary | v2 (default v2)
    const flowResult = await runAnneFlowByMode({
    ...
    if (flowResult.pricingOptions && flowResult.pricingOptions.length > 0) {
      responseMessage = formatPricingResponse(flowResult.pricingOptions as any);
    ...
    const responseMetadata: any = {
      ...
      orchestratorMode: flowResult.mode,
    };
    if (flowResult.metadata) {
      Object.assign(responseMetadata, flowResult.metadata);
    }
