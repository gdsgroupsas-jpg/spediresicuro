...
export async function GET(request: NextRequest) {
  try {
    ...
    if (isSupabaseConfigured()) {
      ...
      const { data: rawProductionStats, error: productionError } = await supabaseAdmin.rpc(
        'get_admin_overview_stats',
        {
          include_test: false,
        }
      ...
      if (!statsError && !productionError && rawStats && rawProductionStats) {
        ...
        console.error('Errore RPC get_admin_overview_stats:', statsError || productionError);
        ...
        const productionShipments = validShipments.filter((s: any) => !isTestShipment(s, userMap));
        const computeShipmentStats = (shipments: any[]) => ({
          ...
          shipmentsDelivered: shipments.filter((s: any) => s.status === 'delivered').length,
          shipmentsFailed: shipments.filter((s: any) => s.status === 'failed').length,
