 ...
 */
export function filterProductionShipments<
  T extends {
    user_id?: string | null;
    created_by?: string | null;
    status?: string | null;
...
>(shipments: T[], userMap?: Map<string, { email?: string | null; name?: string | null }>): T[] {
...
export function createUserMap<
  T extends { id: string; email?: string | null; name?: string | null },
>(users: T[]): Map<string, { email?: string | null; name?: string | null }> {
  const map = new Map();
  users.forEach((user) => {
