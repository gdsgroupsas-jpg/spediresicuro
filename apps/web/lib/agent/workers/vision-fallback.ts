 ...
 */
const UNAVAILABLE_PATTERNS = [/GOOGLE_API_KEY mancante/i, /api.?key.?missing/i, /not.?configured/i];
...
export async function executeVisionWithRetry(
  ...
  while (attempts <= visionFallbackConfig.MAX_RETRIES) {
    ...
    try {
      ...
      const visionResult = await Promise.race([
        ...
        new Promise<never>((_, reject) =>
          setTimeout(
            ...
            visionFallbackConfig.VISION_TIMEOUT_MS
          )
        ),
      ]);
      ...
      logger.warn(
        `⚠️ [Vision] Tentativo ${attempts} fallito: ${lastError.type} - ${lastError.message.substring(0, 100)}`
      );
