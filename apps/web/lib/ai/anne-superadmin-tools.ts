 ...
 */
export async function anneGetAllShipments(filters?: {
  limit?: number;
  offset?: number;
  source_category?: string;
  status?: string;
  ...
  try {
    let query = supabaseAdmin.from('anne_all_shipments_view').select('*');
    ...
    if (filters?.search) {
      query = query.or(
        `tracking_number.ilike.%${filters.search}%,recipient_name.ilike.%${filters.search}%,recipient_city.ilike.%${filters.search}%`
      );
    }
...
> {
  try {
    const { data, error } = await supabaseAdmin.rpc('anne_get_shipments_stats');
...
export async function anneSearchShipments(
  searchTerm: string,
  limit = 50
): Promise<{ success: true; data: ShipmentSearchResult[] } | { success: false; error: string }> {
  try {
    const { data, error } = await supabaseAdmin.rpc('anne_search_shipments', {
 ...
 */
export async function anneGetUnverifiedShipments(
  limit = 50
): Promise<{ success: true; data: AnneShipment[] } | { success: false; error: string }> {
  try {
    const { data, error } = await supabaseAdmin
...
> {
  try {
    ...
    if (monthRevenueResult.data) {
      monthRevenue = monthRevenueResult.data.reduce(
        (sum: number, row: any) => sum + (row.final_price || 0),
        0
      );
    }
