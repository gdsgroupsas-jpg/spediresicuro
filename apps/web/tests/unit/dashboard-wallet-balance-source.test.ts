...
describe('Coerenza: credit-check usa stessa source del dashboard', () => {
  ...
  it('QuickModeForm usa stessa source (user.wallet_balance)', () => {
    ...
    if (quickMode.includes("export { QuickModeForm } from './QuickModeForm.impl';")) {
      const quickModeImpl = readSource('app/dashboard/spedizioni/nuova/QuickModeForm.impl.tsx');
      expect(quickModeImpl).toContain('user?.wallet_balance');
      return;
    }
