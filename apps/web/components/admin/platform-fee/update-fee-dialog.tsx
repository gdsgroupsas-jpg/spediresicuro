...
const FEE_PRESETS = [
  { label: 'Enterprise', value: 0.3, description: 'Volume alto' },
  { label: 'Standard', value: 0.5, description: 'Default' },
  { label: 'Gratis (€0)', value: 0.0, description: 'Nessuna fee' },
 ...
 */
export function UpdateFeeDialog({ userId, currentFee, currentNotes }: UpdateFeeDialogProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const feeDisplay =
        validation.value === null ? 'default (€0.50)' : `€${validation.value.toFixed(2)}`;

      const previousFeeDisplay =
        data.previousFee === null ? 'default (€0.50)' : `€${data.previousFee.toFixed(2)}`;

      const successMsg =
        data.previousFee === null
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Imposta una fee personalizzata per questo utente. Lascia vuoto per tornare al default
              (€0.50).
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="grid gap-4 py-4">
            ...
            {errorMessage && (
              <div className="p-4 bg-red-50 border border-red-200 rounded-lg flex items-start gap-3">
                ...
                <div className="flex-1">
                  <p className="text-sm font-medium text-red-900">❌ Errore: {errorMessage}</p>
              ...
              <div className="flex flex-wrap gap-2">
                {FEE_PRESETS.map((preset) => (
                  ...
                  >
                    {preset.label}
                    {preset.value !== null && (
                      <span className="ml-1 text-gray-500">€{preset.value.toFixed(2)}</span>
                    )}
                  </Button>
                ))}
              </div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-gray-500">Vuoto = torna a default (€0.50)</p>
            </div>
