...
export function CloneSupplierPriceListDialog({
  ...
  const [marginType, setMarginType] = useState<'percent' | 'fixed' | 'none'>('percent');
  ...
  async function handleClone() {
    ...
    try {
      ...
      toast.success(`Listino clonato con successo! (${result.entryCount} tariffe importate)`);

      // Se è stato selezionato un cliente, assegna il listino
      if (selectedClientId) {
        const { resellerAssignPriceListAction } = await import('@/actions/reseller-price-lists');
        ...
        if (assignResult.success) {
          ...
          toast.warning(`Listino clonato ma assegnazione fallita: ${assignResult.error}`);
        }
      }
      ...
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-8">
            ...
            <p className="text-sm text-gray-500">Caricamento listini fornitore...</p>
          </div>
        ) : priceLists.length === 0 ? (
          <div className="p-6 bg-amber-50 border border-amber-200 rounded-lg">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="font-medium text-amber-900">Nessun listino fornitore disponibile</p>
          ...
          <div className="space-y-4 mt-4">
              ...
              >
                ...
                {priceLists.map((pl) => (
                  <option key={pl.id} value={pl.id}>
                    ...
                    {pl.entries && pl.entries.length > 0 ? ` (${pl.entries.length} tariffe)` : ''}
                  </option>
                ))}
              </Select>
              {sourcePriceList && (
                <div className="mt-2 p-3 bg-blue-50 border border-blue-200 rounded-lg text-xs">
                  <p className="font-medium text-blue-900">Informazioni listino selezionato:</p>
                  <ul className="mt-1 space-y-1 text-blue-800">
                    ...
                    <li>• Tariffe: {sourcePriceList.entries?.length || 'N/D'} entries</li>
                  </ul>
                </div>
              )}
            </div>
            ...
            <div>
              ...
              <Select
                id="margin-type"
                value={marginType}
                onChange={(e) => setMarginType(e.target.value as 'percent' | 'fixed' | 'none')}
                disabled={isSaving}
              >
              ...
              <p className="text-xs text-gray-500 mt-1">
                {marginType === 'percent' && 'Applica una percentuale di ricarico sul prezzo base'}
                {marginType === 'fixed' && 'Aggiunge un importo fisso a tutte le tariffe'}
                {marginType === 'none' && 'Copia le tariffe così come sono'}
              </p>
            </div>
            ...
            {marginType !== 'none' && (
              <div>
                ...
                />
                <p className="text-xs text-gray-500 mt-1">
                  {marginType === 'percent' && '20 = +20% sul prezzo base (es. 10€ → 12€)'}
                  {marginType === 'fixed' && '2.50 = +2.50€ a ogni tariffa (es. 10€ → 12.50€)'}
                </p>
              </div>
            )}
            ...
            {marginType !== 'none' && (
              <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="font-medium text-green-900">Anteprima applicazione margine</p>
            ...
            <div>
              ...
              {subUsers.length === 0 && (
                <p className="text-xs text-gray-500 mt-1">
                  Nessun cliente disponibile. Puoi assegnare il listino dopo la creazione.
                </p>
              )}
            </div>
