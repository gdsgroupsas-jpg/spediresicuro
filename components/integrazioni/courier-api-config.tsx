...
export default function CourierAPIConfig() {
  ...
  const hasExistingConfig = existingConfigs.some(
    (c) => c.provider_id === selectedAPI && c.is_active
  );
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-4">
            ...
            {existingConfigs.filter(
              ...
              <p className="text-sm text-gray-500 italic">
                Nessuna configurazione personale attiva.
              </p>
            ) : (
              <div className="space-y-2">
                {existingConfigs
                  ...
                  .map((config) => (
                    <div
                      key={config.id}
                      className="flex items-center justify-between p-3 bg-white rounded-lg border border-gray-200 shadow-sm hover:border-blue-300 transition-colors"
                    >
          ...
          {existingConfigs.some(
            ...
            <div className="bg-gray-50 rounded-xl border border-gray-200 p-4 opacity-75 hover:opacity-100 transition-opacity">
              <h3 className="text-sm font-semibold text-gray-700 mb-3 flex items-center gap-2">
                ...
                Configurazioni Clienti (Supporto)
              </h3>
                ...
                {existingConfigs
                  ...
                  .map((config) => (
                    ...
                    >
                      <div className="flex items-center gap-3 flex-1">
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium text-gray-900 truncate flex items-center gap-2">
                            {config.name}
                            <span className="px-2 py-0.5 rounded-full bg-gray-100 text-xs text-gray-600 font-normal border border-gray-200">
                              {config.created_by}
                            </span>
                          </p>
                          <p className="text-xs text-gray-500 truncate">
                      ...
                      {/* Note: Admin cannot delete client configs from this simplified UI, only view/debug implicitly by seeing them */}
                    </div>
                  ))}
              </div>
            </div>
