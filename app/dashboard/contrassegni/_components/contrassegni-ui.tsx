...
export default function ContrassegniUI({ initialShipments, userId }: ContrassegniUIProps) {
  const [shipments, setShipments] = useState<CashOnDeliveryShipment[]>(initialShipments);
  ...
  useRealtimeShipments({
    ...
    onInsert: (newShipment: any) => {
      // Aggiungi solo se è un contrassegno
      if (newShipment.cash_on_delivery === true && newShipment.deleted !== true) {
        const enriched = enrichShipment(newShipment);
        setShipments((prev) => [enriched, ...prev]);
      }
    },
  ...
  // Helper per arricchire shipment con calcoli
  ...
  async function handlePresoInCarica(shipmentId: string) {
    ...
    try {
      ...
      if (result.success) {
        // Aggiorna localmente (realtime aggiornerà automaticamente)
        setShipments((prev) =>
          prev.map((s) => {
            if (s.id === shipmentId) {
              return {
                ...s,
                contrassegnoInCarica: true,
                paymentStatus: 'in_carica' as const,
              };
            }
            return s;
          })
        ...
        setShipments((prev) =>
          prev.map((s) => {
            if (s.id === shipmentId) {
              return {
                ...s,
                contrassegnoEvaso: true,
                paymentStatus: 'evaso' as const,
              };
            }
            return s;
          })
                ...
                {filteredShipments.map((shipment) => (
                  <tr key={shipment.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      {!shipment.contrassegnoInCarica && !shipment.contrassegnoEvaso && (
                        <button
                          onClick={() => handlePresoInCarica(shipment.id)}
                          disabled={processingId === shipment.id}
                          className="text-orange-600 hover:text-orange-900 disabled:opacity-50"
                        >
                          {processingId === shipment.id ? 'Elaborazione...' : 'Prendi in carica'}
                        </button>
                      )}
                      {shipment.contrassegnoInCarica && !shipment.contrassegnoEvaso && (
                        <button
                          onClick={() => handleEvaso(shipment.id)}
                          disabled={processingId === shipment.id}
