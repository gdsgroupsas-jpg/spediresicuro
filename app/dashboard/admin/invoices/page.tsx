...
import { FileText, Download, Calendar, CheckCircle, Search, Plus, Filter, User } from 'lucide-react';
import { getInvoices, issueInvoice, updateInvoiceStatus, createInvoice } from '@/app/actions/invoices';
...
export default function AdminInvoicesPage() {
  ...
  const handleIssue = async (id: string) => {
    if (!confirm('Sei sicuro di voler emettere questa fattura? Verrà generato il numero progressivo.')) return;
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div className="p-4 border-b border-gray-100 bg-gray-50/50 flex gap-4">
            <div className="relative flex-1 max-w-sm">
                ...
                <input type="text" placeholder="Cerca cliente o numero..." className="w-full pl-9 pr-4 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
            </div>
            {/* Filters */}
        </div>
                  ...
                  <tr key={inv.id} className="hover:bg-gray-50 group transition-colors">
                    ...
                    <td className="px-6 py-4">
                        <div className="flex flex-col">
                            <span className="font-medium text-gray-900">{inv.user?.company_name || inv.user?.name || 'N/A'}</span>
                            <span className="text-xs text-gray-500">{inv.user?.email}</span>
                        </div>
                    </td>
                    <td className="px-6 py-4 text-gray-600">
                        {inv.invoice_date ? format(new Date(inv.invoice_date), 'dd MMM yyyy', { locale: it }) : '-'}
                    </td>
                    ...
                    <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            {inv.status === 'issued' && (
                             ...
                             <Link href={`/dashboard/admin/invoices/${inv.id}`} className="px-2 py-1 text-xs font-medium text-gray-700 bg-white border border-gray-300 rounded hover:bg-gray-50">
                                Dettagli
                            </Link>
                        </div>
                    </td>
