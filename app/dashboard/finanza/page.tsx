...
function FinanceControlRoomContent() {
  ...
  // Generate AI insight
  const aiMessage = fiscalContext
    ? generateAIInsight(fiscalContext)
    : 'Sto analizzando i flussi di cassa in tempo reale...';
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-2 text-lg">
            AI-Powered CFO View • SpedireSicuro Intelligence
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-3 bg-slate-800/50 backdrop-blur-md px-4 py-2 rounded-full border border-slate-700">
            <div className="relative">
              <div
                className={`w-3 h-3 ${isValidating ? 'bg-yellow-500' : 'bg-green-500'} rounded-full animate-pulse`}
              ></div>
              <div
                className={`absolute inset-0 w-3 h-3 ${isValidating ? 'bg-yellow-500' : 'bg-green-500'} rounded-full animate-ping opacity-75`}
              ></div>
            </div>
            <span
              className={`text-sm font-medium ${isValidating ? 'text-yellow-400' : 'text-green-400'}`}
            >
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-red-300 mt-1">
                {dataError.message || 'Errore sconosciuto'}
              </p>
            </div>
          </div>
        </div>
