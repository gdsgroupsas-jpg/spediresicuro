...
export default function FatturaDettaglioPage({ params }: { params: { id: string } }) {
  ...
  if (error || !invoice) return <div className="p-12 text-center text-red-600">{error || 'Fattura non trovata'}</div>;
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-8">
      <Link href="/dashboard/fatture" className="flex items-center text-gray-500 hover:text-gray-900 transition-colors">
        ...
        <div className="bg-gray-50 p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-gray-100">
            <div>
                ...
                <p className="text-gray-500 text-sm mt-1">
                    Emessa il {formatDate(invoice.invoice_date || invoice.created_at)} • Stato: <span className="font-semibold uppercase">{invoice.status}</span>
                </p>
            </div>
            <div className="flex gap-3">
                <button className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 shadow-sm" onClick={() => window.print()}>
        ...
        <div className="p-8 space-y-8">
            ...
            <div className="flex flex-col md:flex-row justify-between gap-8">
                <div className="flex-1">
                    <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Emittente</h3>
                ...
                <div className="flex-1">
                    <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Intestatario</h3>
                    <div className="text-sm text-gray-700 space-y-1">
                        <p className="font-bold text-gray-900 text-lg">{invoice.recipient_name || invoice.user?.company_name || invoice.user?.name}</p>
                        <p>{invoice.recipient_address || invoice.user?.address}</p>
                        <p>{invoice.recipient_city} {invoice.recipient_province} {invoice.recipient_zip}</p>
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {invoice.items?.map((item) => (
                                <tr key={item.id}>
                                    ...
                                    <td className="px-6 py-4 text-right text-gray-600">{formatMoney(item.unit_price)}</td>
                                    <td className="px-6 py-4 text-right text-gray-600">{item.tax_rate}%</td>
                                    <td className="px-6 py-4 text-right font-semibold text-gray-900">{formatMoney(item.total)}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
