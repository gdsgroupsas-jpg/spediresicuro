...
export function ManageWalletDialog({ user, isOpen, onClose, onSuccess }: ManageWalletDialogProps) {
  ...
  useEffect(() => {
    if (isOpen && user) {
      ...
      setTimeout(() => {
        setTransactions([
          { id: '1', amount: 100, type: 'credit', reason: 'Ricarica manuale', created_at: new Date().toISOString(), performed_by_name: 'Admin' },
          { id: '2', amount: -25, type: 'debit', reason: 'Acquisto servizio', created_at: new Date(Date.now() - 86400000).toISOString(), performed_by_name: 'Sistema' },
        ])
