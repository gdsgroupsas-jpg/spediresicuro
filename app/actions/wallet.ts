    ...
    if (analysis.confidence > 0) {
      await supabaseAdmin
        ...
        .update({
          ...
          status:
            analysis.amount && Math.abs(analysis.amount - declaredAmount) > 0.1
              ? 'manual_review'
              : 'pending',
        })
        .eq('id', req.id);
...
export async function approveTopUpRequest(
  ...
  try {
    ...
    if (updateError || !updatedRequest) {
      ...
      console.warn(
        '[TOPUP_APPROVE] UPDATE failed but status still pending/manual_review. Attempting RPC fallback.',
        {
          requestId,
      ...
      try {
        ...
        if (creditError) {
          ...
          return {
            success: false,
            error: 'Status aggiornato ma accredito wallet fallito. Contattare supporto.',
          };
        }
 ...
 * @returns Risultato operazione
 */
export async function deleteTopUpRequest(requestId: string): Promise<{
  success: boolean;
  message?: string;
  error?: string;
}> {
