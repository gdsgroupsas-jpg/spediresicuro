...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (errorMessage.includes('fetch failed') || errorMessage.includes('fetch')) {
      let detailedError =
        'Errore di connessione. Verifica che il Base URL sia raggiungibile e che non ci siano problemi di rete.';
      ...
      if (errorCause?.includes('ENOTFOUND') || errorMessage.includes('ENOTFOUND')) {
        ...
        detailedError =
          `Dominio non trovato: "${domain}" non esiste o non è raggiungibile.\n\n` +
          `💡 Verifica:\n` +
        ...
        detailedError =
          'Connessione rifiutata dal server. Il Base URL potrebbe essere errato o il server non è raggiungibile.';
      } else if (
        ...
        detailedError =
          'Problema certificato SSL. Verifica che il Base URL usi HTTPS e che il certificato sia valido.';
      } else if (errorCause?.includes('ETIMEDOUT') || errorMessage.includes('timeout')) {
        detailedError =
          'Timeout di connessione. Il server potrebbe essere lento o non raggiungibile.';
      }
