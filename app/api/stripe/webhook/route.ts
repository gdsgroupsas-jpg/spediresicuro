...
import { getPaymentTransaction, updatePaymentTransaction } from '@/lib/payments/stripe';
...
async function handleCheckoutSessionCompleted(session: any) {
  ...
  if (tx.status !== 'pending') {
    console.log(`ℹ️ [STRIPE WEBHOOK] Transaction ${transactionId} already processed (status: ${tx.status})`);
    return; // Già processato, ignora
  }

  // 3. Verifica che il pagamento sia stato completato
  if (session.payment_status !== 'paid') {
    console.warn(`⚠️ [STRIPE WEBHOOK] Session ${session.id} not paid (status: ${session.payment_status})`);
