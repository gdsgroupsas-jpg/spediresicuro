...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`🔍 [COURIERS AVAILABLE] Corrieri dalla config API: ${couriers.length}, isSuperadmin: ${isSuperadmin}`);
    ...
    console.log(`🔍 [COURIERS AVAILABLE] Listini attivi (custom+supplier): ${activePriceLists?.length || 0}`);
    ...
    if (activePriceLists && activePriceLists.length > 0) {
      ...
      for (const pl of activePriceLists) {
        ...
        if (codeForMatching) {
          ...
          console.log(`✅ [COURIERS AVAILABLE] Listino attivo (${pl.list_type}): "${pl.name}" → contract_code: ${codeForMatching} (carrier_code: ${carrierCode || 'N/A'})`);
        } else {
          console.warn(`⚠️ [COURIERS AVAILABLE] Listino senza contract_code/carrier_code: "${pl.name}" (${pl.list_type})`);
        }
      }

      if (activeContractCodes.size > 0) {
        console.log(`🔍 [COURIERS AVAILABLE] Contract codes attivi: ${Array.from(activeContractCodes).join(', ')}`);
        ...
        const filteredCouriers = couriers.filter(courier => {
          ...
          if (!courierContractCode) {
            console.warn(`⚠️ [COURIERS AVAILABLE] Corriere senza contract_code: ${courier.displayName || courier.courierName}`);
            return false;
          }
          ...
          if (isActive) {
            console.log(`✅ [COURIERS AVAILABLE] Match contract_code: "${courierContractCode}" (listino) = "${courierContractCode}" (corriere) → mostro "${courier.displayName || courier.courierName}"`);
          } else {
            console.log(`❌ [COURIERS AVAILABLE] Nessun match contract_code: "${courierContractCode}" (corriere) non presente in listini attivi (${Array.from(activeContractCodes).join(', ')})`);
          }
        ...
        console.log(`✅ [COURIERS AVAILABLE] Dopo filtro: ${couriers.length} corrieri con listino attivo`);
      } else {
