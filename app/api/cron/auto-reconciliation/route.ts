...
export async function GET(request: Request) {
  try {
    ...
    const autoMatchResult =
      await reconciliationService.autoReconcilePositiveMargins(
        7,
        SYSTEM_USER_ID
      );
    console.log(
      `[CRON] Auto-matched: ${autoMatchResult.matched}/${autoMatchResult.processed}`
    );

    // 2. Flag margini negativi come discrepancy
    const flaggedCount = await reconciliationService.flagNegativeMargins(
      SYSTEM_USER_ID
    );
