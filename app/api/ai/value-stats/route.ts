...
export async function GET(request: NextRequest) {
  try {
    // âš ï¸ SICUREZZA: Usa requireSafeAuth() per Acting Context
    ...
    console.error('âŒ [Value Stats] Errore:', errorMessage);
 ...
 */
async function calculateValueStats(userId: string): Promise<ValueStats> {
  ...
  if (error) {
    console.error('âŒ [Value Stats] Errore query sessioni:', error);
  ...
  for (const session of sessions || []) {
    try {
      ...
      console.warn('âš ï¸ [Value Stats] Errore parsing stato sessione:', e);
    }
  }
