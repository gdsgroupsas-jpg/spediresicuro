...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`🔍 [QUOTES DB] Utente: ${session.user.email}, isReseller: ${isReseller}, isSuperadmin: ${isSuperadmin}`);
    console.log(`🔍 [QUOTES DB] Corrieri disponibili dalla config: ${availableCouriers.length}`);
    availableCouriers.forEach((c, i) => {
      console.log(`   ${i + 1}. ${c.displayName || c.courierName} (carrierCode: ${c.carrierCode || c.contractCode})`);
    });
    ...
    console.log(`🔍 [QUOTES DB] Listini attivi (custom+supplier) trovati: ${activePriceLists?.length || 0}`);
    ...
    if (isReseller) {
      ...
      if (customPriceLists.length === 0) {
        ...
        return NextResponse.json({
          success: true,
          rates: [],
          message: "Nessun listino personalizzato attivo. Attiva un listino personalizzato per creare spedizioni.",
        });
      }
      console.log(`✅ [QUOTES DB] Reseller: ${customPriceLists.length} listini personalizzati attivi`);
    }
    ...
    if (activePriceLists && activePriceLists.length > 0) {
      ...
      for (const pl of activePriceLists) {
        ...
        if (codeForMatching) {
          ...
          console.log(`✅ [QUOTES DB] Listino attivo (${pl.list_type}): "${pl.name}" → contract_code: ${codeForMatching}`);
        } else {
          // Listino senza contract_code/carrier_code: potrebbe essere un listino globale o legacy
          console.warn(`⚠️ [QUOTES DB] Listino senza contract_code/carrier_code: "${pl.name}" (${pl.list_type}, ID: ${pl.id})`);
      ...
      if (activeContractCodes.size === 0) {
        ...
        return NextResponse.json({
          success: true,
          rates: [],
          message: "I listini attivi non hanno contract_code configurato. Sincronizza i listini o aggiorna i metadata.",
        });
      }

      console.log(`🔍 [QUOTES DB] Contract codes attivi dai listini: ${Array.from(activeContractCodes).join(', ')}`);
      console.log(`🔍 [QUOTES DB] Corrieri disponibili dalla config: ${availableCouriers.length}`);
      availableCouriers.forEach((c, i) => {
        console.log(`   ${i + 1}. ${c.displayName || c.courierName} (carrierCode: ${c.carrierCode || c.contractCode})`);
      });
      ...
      const filteredCouriers = availableCouriers.filter(courier => {
        ...
        const courierContractCode = (courier.contractCode || courier.carrierCode || '').toLowerCase();
        
        if (!courierContractCode) {
          console.warn(`⚠️ [QUOTES DB] Corriere senza contract_code: ${courier.displayName || courier.courierName}`);
          return false;
        }
        ...
        if (isActive) {
          console.log(`✅ [QUOTES DB] Match contract_code: "${courierContractCode}" (listino) = "${courierContractCode}" (corriere) → mostro "${courier.displayName || courier.courierName}"`);
        } else {
          console.log(`❌ [QUOTES DB] Nessun match contract_code: "${courierContractCode}" (corriere) non presente in listini attivi (${Array.from(activeContractCodes).join(', ')})`);
        }
      ...
      console.log(`✅ [QUOTES DB] Dopo filtro: ${availableCouriers.length} carrier_code con listino personalizzato attivo`);
      
      if (availableCouriers.length === 0) {
        return NextResponse.json({
          success: true,
          rates: [],
          message: "Nessun carrier_code corrisponde ai listini personalizzati attivi. Verifica la configurazione.",
        });
      }
      
    } else {
      // Nessun listino personalizzato attivo: non mostrare nessun corriere
      console.log(`⚠️ [QUOTES DB] Nessun listino personalizzato attivo, nessun corriere disponibile`);
      return NextResponse.json({
        success: true,
        rates: [],
    ...
    console.log(`🔍 [QUOTES DB] Inizio calcolo preventivi per ${availableCouriers.length} corrieri filtrati`);
    ...
    for (const courier of availableCouriers) {
      console.log(`🔍 [QUOTES DB] Calcolo preventivo per: ${courier.displayName || courier.courierName} (carrierCode: ${courier.carrierCode || courier.contractCode})`);
      try {
        ...
        if (isReseller || isSuperadmin) {
          ...
          if (bestPriceResult && bestPriceResult.bestPrice) {
            ...
            console.warn(`⚠️ [QUOTES DB] calculateBestPriceForReseller non ha restituito risultati per ${courier.displayName || courier.courierName}`);
    ...
    console.log(`📊 [QUOTES DB] Riepilogo calcolo: ${rates.length} rates calcolati, ${errors.length} errori`);
    ...
    console.log(`✅ [QUOTES DB] Rates finali: ${finalRates.length} (ordinati per prezzo crescente)`);
    console.log(`🔒 [QUOTES DB] ⚠️ IMPORTANTE: Nessuna chiamata API esterna effettuata. Tutti i prezzi calcolati da matrici database.`);
