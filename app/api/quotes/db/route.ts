...
import { calculateQuoteAction } from '@/actions/price-lists';
...
export async function POST(request: NextRequest) {
  try {
      ...
      try {
        ...
        if (isReseller) {
          ...
          if (bestPriceResult && bestPriceResult.bestPrice) {
            ...
            quoteResult._configId = bestPriceResult.apiSource === 'reseller' 
              ? 'reseller_config' 
              : 'master_config';
          }
        } else {
        ...
        if (quoteResult && quoteResult.finalPrice) {
          ...
          const supplierPrice = quoteResult.totalCost || quoteResult.basePrice || quoteResult.finalPrice;
          rates.push({
            ...
            base_price: quoteResult.basePrice?.toString() || quoteResult.finalPrice.toString(),
    ...
    if (isSuperadmin && verifyCosts) {
      try {
        const { testSpedisciOnlineRates } = await import('@/actions/spedisci-online-rates');
        const { getAllUserSpedisciOnlineConfigs } = await import('@/lib/actions/spedisci-online');
        ...
        if (configsResult.success && configsResult.configs && configsResult.configs.length > 0) {
          ...
          const apiPromises = configsResult.configs.map(async (config) => {
            try {
              ...
              console.error(`❌ [QUOTES DB] Errore API per config ${config.configId}:`, error);
              return [];
            }
          });
