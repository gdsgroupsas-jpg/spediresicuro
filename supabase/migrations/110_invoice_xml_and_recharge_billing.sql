...
ALTER TABLE invoices
ADD COLUMN invoice_type TEXT DEFAULT 'shipment' CHECK (
    invoice_type IN ('shipment', 'recharge', 'periodic', 'manual')
  );
...
COMMENT ON TABLE invoice_recharge_links IS 'Collegamento ricariche wallet → fatture. Supporta fatture che includono più ricariche.';
-- ============================================
...
CREATE TABLE IF NOT EXISTS invoice_generation_rules (
  ...
  generation_type TEXT NOT NULL CHECK (
    generation_type IN ('automatic', 'manual', 'periodic')
  ),
  -- Per periodic: frequenza
  period_frequency TEXT CHECK (
    period_frequency IN ('monthly', 'quarterly', 'yearly')
  ),
  -- Per periodic: giorno del mese/trimestre
  period_day INTEGER CHECK (
    period_day >= 1
    AND period_day <= 31
  ),
  -- Filtri ricariche
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_invoice_generation_rules_unique_active ON invoice_generation_rules(user_id, generation_type)
WHERE is_active = true;
CREATE INDEX IF NOT EXISTS idx_invoice_generation_rules_user ON invoice_generation_rules(user_id);
CREATE INDEX IF NOT EXISTS idx_invoice_generation_rules_active ON invoice_generation_rules(is_active)
WHERE is_active = true;
COMMENT ON TABLE invoice_generation_rules IS 'Regole per generazione automatica/manuale/periodica fatture ricariche.';
-- ============================================
...
v_caller_id UUID;
v_is_admin BOOLEAN;
BEGIN -- 🔒 SICUREZZA: Verifica autorizzazione chiamante
-- Se chiamato da utente autenticato (non system), verifica permessi
v_caller_id := auth.uid();
IF v_caller_id IS NOT NULL THEN -- Verifica se chiamante è admin o se sta fatturando per se stesso
SELECT EXISTS(
    SELECT 1
    FROM users
...
IF NOT v_is_admin
AND v_caller_id != p_user_id THEN RAISE EXCEPTION 'Non autorizzato: puoi fatturare solo per te stesso o devi essere admin';
END IF;
END IF;
-- Se v_caller_id IS NULL, è chiamata da system (webhook) → OK (validazione ownership nelle transazioni)
-- Validazione
IF array_length(p_transaction_ids, 1) IS NULL
OR array_length(p_transaction_ids, 1) = 0 THEN RAISE EXCEPTION 'Nessuna transazione specificata';
END IF;
...
WHERE wt.id = ANY(p_transaction_ids)
  ...
  AND NOT EXISTS (
    SELECT 1
    FROM invoice_recharge_links irl
    WHERE irl.wallet_transaction_id = wt.id
  ) LOOP v_total_amount := v_total_amount + v_tx.amount;
END LOOP;
IF v_total_amount = 0 THEN RAISE EXCEPTION 'Nessuna ricarica valida da fatturare (già fatturate o importo zero)';
END IF;
...
VALUES (
    v_invoice_id,
    CASE
      WHEN p_invoice_type = 'periodic'
      AND p_period_start IS NOT NULL
      AND p_period_end IS NOT NULL THEN 'Ricariche wallet dal ' || TO_CHAR(p_period_start, 'DD/MM/YYYY') || ' al ' || TO_CHAR(p_period_end, 'DD/MM/YYYY')
      ELSE 'Ricarica wallet - ' || array_length(p_transaction_ids, 1)::TEXT || ' transazione/i'
    END,
    1,
    v_subtotal,
    22.00,
...
COMMENT ON FUNCTION generate_invoice_from_recharges IS 'Genera fattura da ricariche wallet. Supporta fatture singole o periodiche.';
-- ============================================
-- STEP 5: RLS Policies
-- ============================================
...
CREATE POLICY invoice_recharge_links_select ON invoice_recharge_links FOR
SELECT USING (
    EXISTS (
      SELECT 1
      FROM invoices i
...
CREATE POLICY invoice_generation_rules_select ON invoice_generation_rules FOR
SELECT USING (
    user_id = auth.uid()
    OR EXISTS (
      SELECT 1
...
CREATE POLICY invoice_generation_rules_modify ON invoice_generation_rules FOR ALL USING (
  EXISTS (
    SELECT 1
    FROM users u
    WHERE u.id = auth.uid()
