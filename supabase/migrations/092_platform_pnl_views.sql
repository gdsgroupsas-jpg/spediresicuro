-- ============================================
-- MIGRATION: 092_platform_pnl_views.sql
-- DESCRIZIONE: Viste per P&L e reporting finanziario piattaforma
-- DATA: 2026-01-07
-- CRITICITÀ: P1 - REPORTING
...
-- 3. v_reseller_monthly_platform_usage - Usage per reseller
...
COMMENT ON VIEW v_platform_daily_pnl IS 
  'P&L giornaliero per spedizioni con contratti piattaforma, raggruppato per corriere.
...
COMMENT ON VIEW v_platform_monthly_pnl IS 
  'P&L mensile aggregato per contratti piattaforma.
...
COMMENT ON VIEW v_reseller_monthly_platform_usage IS 
  'Usage mensile per ogni reseller/user che usa contratti piattaforma.
...
COMMENT ON VIEW v_platform_margin_alerts IS 
  'Spedizioni con margini anomali: negativi, troppo bassi (< 5%), o troppo alti (> 50%).
...
SELECT 
  ...
  CASE 
    WHEN ppc.provider_invoice_amount IS NOT NULL 
    THEN ppc.provider_invoice_amount - ppc.provider_cost
    ELSE NULL
  END AS invoice_discrepancy,
  
  -- Età record (giorni da creazione)
  EXTRACT(DAY FROM NOW() - ppc.created_at)::INTEGER AS age_days,
...
ORDER BY 
  CASE ppc.reconciliation_status
    WHEN 'discrepancy' THEN 0  -- Priorità: discrepanze prima
    ELSE 1
  END,
  ppc.created_at ASC;  -- FIFO: i più vecchi prima

COMMENT ON VIEW v_reconciliation_pending IS 
  'Spedizioni in attesa di riconciliazione o con discrepanze.
   Ordinato per priorità (discrepanze prima) e età (FIFO).';

-- ============================================
-- MATERIALIZED VIEW per Dashboard (opzionale)
-- ============================================

-- Se le query sono lente, creare materialized view:
/*
CREATE MATERIALIZED VIEW mv_platform_monthly_pnl AS
SELECT * FROM v_platform_monthly_pnl;

CREATE UNIQUE INDEX ON mv_platform_monthly_pnl(month);

-- Refresh (da schedulare):
REFRESH MATERIALIZED VIEW CONCURRENTLY mv_platform_monthly_pnl;
*/

-- ============================================
-- GRANTS (le views ereditano RLS dalle tabelle sottostanti)
-- ============================================

-- Nota: Le views sono accessibili solo a chi può accedere alle tabelle sottostanti.
-- Dato che platform_provider_costs ha RLS solo per superadmin,
-- queste views sono automaticamente ristrette.
...
BEGIN
  RAISE NOTICE '========================================';
  RAISE NOTICE '✅ Migration 092 completata con successo';
  RAISE NOTICE '';
  RAISE NOTICE '📊 VIEWS CREATE:';
  RAISE NOTICE '   1. v_platform_daily_pnl - P&L giornaliero per corriere';
  RAISE NOTICE '   2. v_platform_monthly_pnl - P&L mensile aggregato';
  RAISE NOTICE '   3. v_reseller_monthly_platform_usage - Usage per reseller';
  RAISE NOTICE '   4. v_platform_margin_alerts - Alert margini anomali';
  RAISE NOTICE '   5. v_reconciliation_pending - Spedizioni da riconciliare';
  RAISE NOTICE '';
  RAISE NOTICE '📈 USO:';
  RAISE NOTICE '   SELECT * FROM v_platform_daily_pnl WHERE date >= CURRENT_DATE - 7;';
  RAISE NOTICE '   SELECT * FROM v_platform_margin_alerts;';
  RAISE NOTICE '';
  RAISE NOTICE '⚠️  NOTA: Le views ereditano RLS da platform_provider_costs,';
  RAISE NOTICE '   quindi sono accessibili solo a SuperAdmin.';
  RAISE NOTICE '========================================';
END $$;
