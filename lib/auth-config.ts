...
function validateOAuthConfig() {
  ...
  if (
    ...
    warnings.push(
      "⚠️ NEXTAUTH_URL non configurato - consigliato in produzione"
    );
  }
  ...
  if (hasGoogle && process.env.NODE_ENV === "production") {
    ...
    console.log(
      "📝 [AUTH CONFIG] Verifica che il callback URL sia configurato in Google Console:"
    );
...
function getNextAuthUrl(): string {
  ...
  if (process.env.VERCEL_URL) {
    ...
    if (
      ...
      console.warn(
        "⚠️ [AUTH] NEXTAUTH_URL punta a localhost, uso VERCEL_URL invece"
      );
      return vercelUrl;
    }
    ...
    if (
      ...
      console.log(
        "✅ [AUTH] Usando NEXTAUTH_URL configurato:",
        process.env.NEXTAUTH_URL
      );
...
if (process.env.NODE_ENV === "production" && !nextAuthSecret) {
  ...
  console.error(
    '❌ [AUTH CONFIG] Questo causerà l\'errore "Configuration" in NextAuth.'
  );
  console.error(
      ...
      async authorize(
        ...
        try {
          ...
          if (user) {
            ...
            console.log(
              "❌ [AUTH] Utente non trovato o password errata per:",
              credentials.email
            );
            ...
            if (
              ...
              console.warn(
                "⚠️ [AUTH] ATTENZIONE: Utente demo non trovato dopo inizializzazione!"
              );
            }
          }
        } catch (error: any) {
  ...
  callbacks: {
    async signIn({ user, account, profile }: any) {
      ...
      if (account?.provider !== "credentials" && user?.email) {
        try {
          ...
          const { findUserByEmail, createUser, updateUser } = await import(
            "@/lib/database"
          );
          ...
          if (!dbUser) {
            ...
            console.log(
              "🔄 [NEXTAUTH] Aggiornamento utente esistente con provider OAuth"
            );
          ...
          try {
            ...
            console.warn(
              "⚠️ [SUPABASE] Errore sincronizzazione profilo:",
      ...
      if (user?.email && AUTHORIZED_SUPERADMINS.includes(user.email)) {
        try {
          console.log(
            "👑 [AUTO-PROMOTE] Email autorizzata rilevata:",
            user.email
          );
          ...
          if (existingUser?.account_type !== "superadmin") {
            console.log(
              "🔄 [AUTO-PROMOTE] Promozione a superadmin in corso..."
            );
            ...
            if (!updateError) {
              console.log(
                "✅ [AUTO-PROMOTE] Utente promosso a superadmin automaticamente"
              );
              ...
              try {
                ...
                console.warn(
                  "⚠️ [AUTO-PROMOTE] Errore audit log (non critico)"
                );
              }
            } else {
              console.error(
                "❌ [AUTO-PROMOTE] Errore durante promozione:",
                updateError
              );
            }
          } else {
          ...
          console.error(
            "❌ [AUTO-PROMOTE] Errore auto-promozione:",
            error?.message
          );
    ...
    async jwt({ token, user, account }: any) {
      ...
      if (user) {
        ...
        if (user.email && typeof window === "undefined") {
          ...
          try {
            ...
            const { data: userData, error } = await supabaseAdmin
              .from("users")
              .select(
                "id, is_reseller, reseller_role, parent_id, wallet_balance, account_type"
              )
              .eq("email", user.email)
              .single();
            ...
            if (!error && userData) {
              ...
              token.wallet_balance =
                parseFloat(userData.wallet_balance || "0") || 0;
        ...
        console.log(
          "🔄 [NEXTAUTH] jwt callback - aggiornamento token esistente:",
          {
            id: token.id,
            email: token.email,
            role: token.role,
    ...
    async redirect({ url, baseUrl }: any) {
      ...
      if (!url || url === "/" || url === "") {
        ...
        console.log(
          "⚠️ [NEXTAUTH] URL vuoto o /, redirect fail-safe a onboarding:",
          redirectUrl
        );
        return redirectUrl;
      }
      ...
      if (url === "/login" || url.startsWith("/login")) {
        ...
        console.log(
          "⚠️ [NEXTAUTH] URL è /login, reindirizzo a dashboard:",
          redirectUrl
        );
        return redirectUrl;
      }
      ...
      try {
        ...
        if (urlObj.origin === baseUrlObj.origin) {
          console.log(
            "✅ [NEXTAUTH] Redirect a URL assoluto stesso dominio:",
            url
          );
          return url;
        }
        ...
        if (
          urlObj.origin.includes("localhost") &&
          process.env.NODE_ENV === "production"
        ) {
          const correctedUrl = url.replace(urlObj.origin, baseUrlObj.origin);
          console.log(
            "⚠️ [NEXTAUTH] URL corretto da localhost a produzione:",
            correctedUrl
          );
          return correctedUrl;
        }
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
      ...
      console.warn(
        "⚠️ [AUTH CONFIG] NEXTAUTH_SECRET non configurato. Usando secret di sviluppo."
      );
