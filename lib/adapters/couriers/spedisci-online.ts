...
export class SpedisciOnlineAdapter extends CourierAdapter {
  ...
  constructor(credentials: SpedisciOnlineCredentials) {
    ...
    if (!credentials.api_key) {
      throw new Error(
        "Spedisci.Online: API Key mancante per la creazione LDV."
      );
    }
    ...
    const baseUrl =
      credentials.base_url || "https://api.spedisci.online/api/v2";
    ...
    if (!normalizedBaseUrl.includes("spedisci.online")) {
      console.warn(
        "⚠️ [SPEDISCI.ONLINE] Base URL potrebbe essere errato:",
        normalizedBaseUrl
      );
    }
    if (!normalizedBaseUrl.includes("/api/v2")) {
      console.warn(
        "⚠️ [SPEDISCI.ONLINE] Base URL potrebbe mancare /api/v2:",
        normalizedBaseUrl
      );
    }
    ...
    console.log("🔧 [SPEDISCI.ONLINE] Base URL configurato:", {
      ...
      isProduction:
        this.BASE_URL.includes("api.spedisci.online") &&
        !this.BASE_URL.includes("demo"),
    });
  }
  ...
  async createShipment(
    ...
    try {
      ...
      try {
        ...
        if (result.success) {
          ...
          console.log("📦 [SPEDISCI.ONLINE] ShippingLabel creato:", {
            ...
            metadata_keys: shippingLabel.metadata
              ? Object.keys(shippingLabel.metadata)
              : [],
        ...
        if (is401 && this.BASE_URL.includes("/api/v2")) {
          ...
          try {
            ...
            console.warn(
              "⚠️ [SPEDISCI.ONLINE] Anche endpoint /v1 fallito:",
              v1Error.message
            );
          }
        }
      ...
      try {
        const legacyPayload = this.mapToSpedisciOnlineFormat(
          data,
          contractCode
        );
        ...
        if (result.success) {
          return {
            tracking_number:
              result.tracking_number || this.generateTrackingNumber(),
            label_url: result.label_url,
            label_pdf: result.label_pdf
              ? Buffer.from(result.label_pdf, "base64")
              : undefined,
      ...
      const courier = (data.corriere || data.courier_id || "")
        .toLowerCase()
        .trim();
      if (
        !this.CONTRACT_MAPPING ||
        Object.keys(this.CONTRACT_MAPPING).length === 0
      ) {
  ...
  private async createShipmentJSON(
    ...
    let baseUrlNormalized = this.BASE_URL.endsWith("/")
      ? this.BASE_URL
      : `${this.BASE_URL}/`;
    ...
    if (apiVersion === "v1" && baseUrlNormalized.includes("/api/v2/")) {
      ...
      console.log(
        "🔄 [SPEDISCI.ONLINE] Usando endpoint API v1:",
        baseUrlNormalized
      );
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log("📦 [SPEDISCI.ONLINE] Analisi campi:", {
          ...
          response_trackingNumber:
            result.trackingNumber || result.tracking_number,
          // Cerca anche in oggetti annidati
          packages_first: result.packages?.[0],
          has_nested_shipmentId: !!(
            result.shipment?.shipmentId || result.data?.shipmentId
          ),
        });
        console.log(
          "═══════════════════════════════════════════════════════════════════════════"
        ...
        console.log(
          "🔍 [SPEDISCI.ONLINE] ⚠️⚠️⚠️ VERIFICA SHIPMENTID NELLA RISPOSTA API ⚠️⚠️⚠️:",
          {
        ...
        const labelData =
          ...
          result.label_pdf ||
          result.label ||
          result.labelPdf; // Base64 encoded
        ...
        if (shipmentId) {
          console.log(
            "✅ [SPEDISCI.ONLINE] shipmentId (increment_id) trovato:",
            {
              shipmentId,
          ...
          console.error(
            "❌ [SPEDISCI.ONLINE] shipmentId NON TROVATO nella risposta e impossibile estrarlo dal tracking!",
            {
              ...
              warning:
                "La cancellazione futura potrebbe non funzionare correttamente",
            }
          );
        }
      ...
      throw new Error(
        `Spedisci.Online Error (${response.status}): ${errorMessage}`
      );
    } catch (error: any) {
      ...
      throw new Error(
        `Errore di connessione Spedisci.Online: ${error.message}`
      );
    }
  }
  ...
  private async uploadCSV(csvContent: string): Promise<SpedisciOnlineResponse> {
    ...
    for (const endpoint of uploadEndpoints) {
      ...
      const baseUrlNormalized = this.BASE_URL.endsWith("/")
        ? this.BASE_URL
        : `${this.BASE_URL}/`;
      ...
      try {
        ...
        if (response.status !== 404) {
          ...
          throw new Error(
            `Upload CSV fallito (${response.status}): ${errorText}`
          );
        }
        ...
        lastError = error;
        console.warn(
          `⚠️ [SPEDISCI.ONLINE] Errore upload su ${url}:`,
          error.message
        );
      }
    }
    ...
    console.error(
      "❌ [SPEDISCI.ONLINE] Tutti gli endpoint upload CSV hanno fallito"
    );
    throw (
      lastError ||
      new Error("Spedisci.Online: Nessun endpoint upload CSV valido trovato")
    );
  }
  ...
  private findContractCode(
    ...
    if (
      ...
      console.warn(
        "⚠️ [SPEDISCI.ONLINE] Nessun contratto configurato o corriere mancante"
      );
      return undefined;
    }
    ...
    for (const [baseName, aliases] of Object.entries(courierAliases)) {
      if (
        aliases.some(
          (alias) => courier.includes(alias) || alias.includes(courier)
        )
      ) {
    ...
    console.log(
      ...
      Object.keys(this.CONTRACT_MAPPING).map(
        (k) => `${k} -> ${this.CONTRACT_MAPPING[k]}`
      )
    );
    ...
    for (const [contractCode, courierName] of Object.entries(
      ...
      // Match esatto
      if (
        normalizedCourierName === courier ||
        normalizedCourierName === normalizedCourier
      ) {
        console.log(
      ...
      const courierWords = courier
        .split(/\s+/)
        .filter((w: string) => w.length > 2); // Parole significative
      ...
      if (
        courierWords.some((word: string) =>
          normalizedCourierName.includes(word.toLowerCase())
        ) ||
        courierNameWords.some((word: string) =>
          courier.includes(word.toLowerCase())
        )
      ) {
        console.log(
    ...
    for (const [contractCode] of Object.entries(this.CONTRACT_MAPPING)) {
      ...
      if (
        ...
        console.log(
          `✅ Codice contratto trovato (match chiave) per ${courier}: ${contractCode}`
        );
        return contractCode;
      }
    }
    ...
    for (const [contractCode] of Object.entries(this.CONTRACT_MAPPING)) {
      ...
      if (
        ...
        if (
          ...
          console.log(
            `✅ Codice contratto trovato (parziale) per ${courier}: ${contractCode}`
          );
          return contractCode;
        }
      }
    ...
    for (const [contractCode, courierName] of Object.entries(
      ...
      const courierNameFirstWord = normalizedCourierName
        .split(/\s+/)[0]
        .toLowerCase();
  ...
  private mapToOpenAPIFormat(
    ...
    const senderCity =
      "sender_city" in data
        ? data.sender_city
        : data.mittente?.citta || data.sender?.citta || "";
    const senderZip =
      "sender_zip" in data
        ? data.sender_zip
        : data.mittente?.cap || data.sender?.cap || "";
    ...
    const senderEmail =
      "sender_email" in data
        ? data.sender_email
        : data.mittente?.email || data.sender?.email || "";
    ...
    const weight =
      "weight" in data ? Number(data.weight) || 1 : Number(data.peso) || 1;
    const length =
      "length" in data
        ? Number(data.length) || 10
        : Number(data.dimensioni?.lunghezza) || 10;
    const width =
      "width" in data
        ? Number(data.width) || 10
        : Number(data.dimensioni?.larghezza) || 10;
    const height =
      "height" in data
        ? Number(data.height) || 10
        : Number(data.dimensioni?.altezza) || 10;
    ...
    if ("codValue" in data && data.codValue != null) {
      ...
      "contrassegnoAmount" in data &&
      data.contrassegnoAmount != null
    ) {
      codAmount = parseFloat(String(data.contrassegnoAmount)) || 0;
    } else if (
      "cash_on_delivery_amount" in data &&
      data.cash_on_delivery_amount != null
    ) {
      codAmount = Number(data.cash_on_delivery_amount) || 0;
    } else if (
      "contrassegno" in data &&
      typeof data.contrassegno === "number"
    ) {
    ...
    const insuranceValue =
      "declared_value" in data && data.declared_value
        ...
        : "assicurazione" in data &&
          data.assicurazione &&
          typeof data.assicurazione === "number"
        ? Number(data.assicurazione)
        : "insurance" in data &&
          data.insurance &&
          typeof data.insurance === "number"
  ...
  private mapToSpedisciOnlineFormat(
    ...
    const senderName =
      "sender_name" in data
        ? data.sender_name
        : data.mittente?.nome || data.sender?.nome || "";
    const tracking =
      "tracking_number" in data ? data.tracking_number : data.tracking || "";
    const finalPrice =
      "final_price" in data ? data.final_price : data.prezzoFinale || 0;
  ...
  private generateCSV(payload: SpedisciOnlineShipmentPayload): string {
    ...
    const contrassegnoValue =
      payload.codValue > 0 ? String(payload.codValue) : "0";
  ...
  async getIncrementIdByTracking(
    ...
    try {
      ...
      console.log(
        `📡 [SPEDISCI.ONLINE] GET /tracking/${trackingNumber} response:`,
        {
          status: response.status,
          statusText: response.statusText,
        }
      ...
      if (response.ok) {
        ...
        console.log(
          "📦 [SPEDISCI.ONLINE] Risposta GET /tracking/{ldv} COMPLETA:"
        );
        console.log(JSON.stringify(result, null, 2));
        console.log(
          "═══════════════════════════════════════════════════════════════════════════"
        ...
        if (incrementId) {
          const incrementIdNum =
            typeof incrementId === "string"
              ? parseInt(incrementId, 10)
              : incrementId;

          console.log(
            `✅ [SPEDISCI.ONLINE] increment_id trovato in risposta tracking!`,
            {
              trackingNumber,
        ...
        console.warn(
          `⚠️ [SPEDISCI.ONLINE] Spedizione trovata ma increment_id NON disponibile`,
          {
            trackingNumber,
        ...
        console.log(
          `ℹ️ [SPEDISCI.ONLINE] Spedizione ${trackingNumber} non trovata (404)`,
          {
  ...
  async cancelShipmentOnPlatform(
    ...
    try {
      ...
      if (isPureNumber) {
        ...
        console.log(
          "✅ [SPEDISCI.ONLINE] Usando increment_id diretto (numero puro):",
          incrementId
        );
      } else {
        // ⚠️ PRIORITÀ 2: Prova a recuperare increment_id da Spedisci.Online usando il tracking
        console.log(
          "🔍 [SPEDISCI.ONLINE] Tentativo recupero increment_id da Spedisci.Online..."
        );
        const incrementIdResult = await this.getIncrementIdByTracking(
          trackingNumber
        );
      ...
      if (!incrementId || incrementId === 0) {
        console.error(
          "❌ [SPEDISCI.ONLINE] Impossibile estrarre increment_id valido da:",
          {
            trackingNumber,
      ...
      if (response.ok) {
        ...
        console.log(
          "✅ [SPEDISCI.ONLINE] Spedizione cancellata:",
          trackingNumber
        );
        return {
          success: true,
          message:
            result.message ||
            "Spedizione cancellata con successo su Spedisci.Online",
        };
      }
      ...
      if (response.status === 404 || response.status === 400) {
        const statusMsg =
          response.status === 404
            ? "non trovata"
            : "bad request (increment_id non valido?)";
      ...
      return {
        ...
        error:
          error?.message ||
          "Errore durante la cancellazione su Spedisci.Online",
      };
    }
  }
