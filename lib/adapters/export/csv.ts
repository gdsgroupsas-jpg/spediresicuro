...
export class CSVExporter {
  ...
  static export<T extends Record<string, any>>(
    ...
    const { delimiter = ',', includeHeaders = true, encoding = 'utf-8-bom' } = options;
  ...
  static exportLDV(shipment: any, options?: CSVOptions): string {
    const ldvData = [
      { campo: 'Tracking Number', valore: shipment.tracking_number },
      {
        ...
        valore: new Date(shipment.created_at).toLocaleDateString('it-IT'),
      },
      { campo: '', valore: '' },
      ...
      {
        campo: 'Città',
        valore: `${shipment.sender_zip} ${shipment.sender_city} (${shipment.sender_province})`,
      },
      ...
      {
        campo: 'Città',
        valore: `${shipment.recipient_zip} ${shipment.recipient_city} (${shipment.recipient_province})`,
      },
      ...
      {
        campo: 'Dimensioni (cm)',
        valore: `${shipment.length}x${shipment.width}x${shipment.height}`,
      },
      {
      ...
      {
        campo: 'Contrassegno',
        valore: shipment.cash_on_delivery ? `€${shipment.cash_on_delivery_amount}` : 'No',
      },
