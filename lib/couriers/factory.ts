...
export async function getShippingProvider(
  ...
  try {
    ...
    const config = await getCourierConfigForUser(userId, providerId, specificConfigId);
 ...
 */
export async function isProviderAvailable(userId: string, providerId: string): Promise<boolean> {
  try {
