...
const WooCommerceSchema = z.object({
  ...
  api_key: z
    .string()
    .min(1, 'Consumer Key obbligatorio')
    .startsWith('ck_', 'Consumer Key deve iniziare con ck_'),
  api_secret: z
    .string()
    .min(1, 'Consumer Secret obbligatorio')
    .startsWith('cs_', 'Consumer Secret deve iniziare con cs_'),
});
...
const ShopifySchema = z.object({
  ...
  access_token: z
    .string()
    .min(1, 'Access Token obbligatorio')
    .startsWith('shpat_', 'Access Token deve iniziare con shpat_'),
});
...
function normalizeCredentials(provider: string, data: any) {
  switch (provider.toLowerCase()) {
    case 'shopify':
      ...
      const shopUrl: string = String(data.store_url || '')
        .replace(/^https?:\/\//, '')
  ...
  try {
    ...
    if (error instanceof z.ZodError) {
      return {
        success: false,
        error: error.errors
          .map((e: z.ZodIssue) => `${String(e.path.join('.'))}: ${e.message}`)
          .join(', '),
      };
    }
    return {
  ...
  try {
    ...
    if (useLocalFallback) {
      ...
      const existingIndex = integrations.findIndex(
        (i: any) => i.platform === provider.toLowerCase()
      );
