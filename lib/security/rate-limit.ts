...
function getRedis(): Redis | null {
  ...
  if (!url || !token) {
    console.warn('⚠️ [RATE-LIMIT] Redis non configurato - fallback in-memory');
    return null;
  }
  
  try {
    redis = new Redis({ url, token });
    console.log('✅ [RATE-LIMIT] Redis connesso');
    return redis;
  } catch (error) {
    console.error('❌ [RATE-LIMIT] Errore init Redis - fallback in-memory');
    return null;
  }
}
...
function rateLimitInMemory(
  ...
  current.count++;
  return { allowed: true, remaining: limit - current.count, resetAt: current.expiresAt };
}
