...
export function encryptCredential(plaintext: string): string {
  ...
  if (!process.env.ENCRYPTION_KEY?.trim()) {
    if (process.env.NODE_ENV === 'production') {
      ...
      console.error(
        '❌ [SECURITY P0] ENCRYPTION_KEY non configurata in produzione. Operazione bloccata.'
      );
      throw new Error(
        'ENCRYPTION_KEY_MISSING: Impossibile salvare credenziali in modo sicuro. Configura ENCRYPTION_KEY su Vercel.'
      );
    } else {
      // Development: permetti plaintext per testing locale (con warning)
      console.warn(
        '⚠️ [DEV] ENCRYPTION_KEY non configurata. Credenziali salvate in chiaro (solo sviluppo).'
      );
      return plaintext;
    }
  }
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'production') {
      throw new Error(
        'ENCRYPTION_FAILED: Errore durante la criptazione. Credenziali non salvate per sicurezza.'
      );
    }
...
export function decryptCredential(encryptedData: string): string {
  ...
  try {
    ...
    console.warn(
      `⚠️ [ENCRYPTION] Decrypt fallito con chiave corrente - hash: ${dataHash}, tentativo con legacy...`
    );
    ...
    if (legacyKey) {
      try {
        ...
        console.error(
          `❌ [ENCRYPTION] Decrypt fallito anche con chiave legacy - hash: ${dataHash}`,
          {
    ...
    throw new Error(
      "CREDENTIAL_DECRYPT_FAILED: Impossibile decriptare credenziali. La chiave di criptazione potrebbe essere stata cambiata. Verifica ENCRYPTION_KEY su Vercel (Production/Preview devono essere identiche) o re-inserisci le credenziali dell'integrazione."
    );
  }
}
