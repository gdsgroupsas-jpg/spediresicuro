...
function translateValidationError(error: string, field?: string): HumanMessage {
  ...
  if (errorLower.includes('cap') || errorLower.includes('zip')) {
    return {
      message: 'Il CAP deve essere un numero di 5 cifre valido.',
      actionable: true,
      severity: 'warning',
    };
  }
...
function translatePreflightError(errors: string[]): HumanMessage {
  ...
  return {
    message: 'Ci sono dati mancanti per completare la prenotazione.',
    actionable: true,
    severity: 'warning',
  };
}
...
function translateLowConfidence(confidenceScore: number): HumanMessage {
  ...
  if (confidenceScore >= 50) {
    return {
      message: 'I dati non sono completamente chiari.',
      actionable: true,
      severity: 'warning',
    };
  }
  return {
    message: 'I dati non sono chiari. Puoi riformulare la richiesta in modo piu specifico?',
    actionable: true,
    severity: 'warning',
  };
}
  ...
  try {
    ...
    if (state.booking_result?.status === 'failed') {
      ...
      if (errorCode === 'INSUFFICIENT_CREDIT') {
        return {
          message: 'Il saldo del wallet non e sufficiente per questa spedizione. Vuoi ricaricare?',
          actionable: true,
          severity: 'error',
        };
      }
      ...
      return {
        message: state.booking_result.user_message || 'La prenotazione non e andata a buon fine.',
        actionable: true,
        severity: 'error',
      };
    }
