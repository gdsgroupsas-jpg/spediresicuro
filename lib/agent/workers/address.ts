...
export async function addressWorker(
  ...
  try {
    ...
    if (!messageText.trim()) {
      ...
      return {
        clarification_request:
          'Non ho ricevuto informazioni. Puoi indicarmi CAP, città e peso del pacco?',
        next_step: 'END',
      };
    }
    ...
    logger.log(
      `📍 [Address Worker] Estratto: ${extractedAnything ? 'sì' : 'no'}, campi mancanti: ${updatedDraft.missingFields.length}`
    );
    ...
    return {
      clarification_request:
        'Mi dispiace, non sono riuscita a elaborare i dati. Puoi riprovare indicando CAP, città e peso?',
      next_step: 'END',
...
export function processAddressSync(
  ...
  const { updatedDraft, missingFields, extractedAnything, readyForPricing } = processAddressCore(
    message,
    existingDraft
  );
  ...
  return {
    shipmentDraft: updatedDraft,
    missingFields,
    clarificationQuestion: readyForPricing
      ? undefined
      : generateClarificationQuestion(missingFields),
