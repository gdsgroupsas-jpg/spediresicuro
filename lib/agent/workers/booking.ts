  ...
  try {
    ...
    if (!credentials) {
      return {
        ...
        user_message:
          'Configurazione corriere non trovata. Configura le credenziali in Integrazioni.',
      };
    }
    ...
    if (!connected) {
      return {
        ...
        user_message:
          'Impossibile connettersi al servizio di spedizione. Riprova tra qualche minuto.',
...
async function getBookingCredentials(userId: string): Promise<any | null> {
  try {
    ...
    defaultLogger.warn(
      '⚠️ [Booking] Nessuna configurazione Spedisci.Online trovata nel DB per utente:',
      userEmail
    );
    ...
    defaultLogger.warn(
      '⚠️ [Booking] Nessuna configurazione Spedisci.Online disponibile (né DB né ENV)'
    );
    return null;
  } catch (error: any) {
...
function mapAdapterError(error?: string): BookingErrorCode {
  ...
  if (
    ...
    errorLower.includes('idempotent')
  ) {
