...
Se un dato non è presente, usa null.`;
      ...
      return {
        ...
        destinationZip:
          extracted.destinationZip ?? existingDetails?.destinationZip,
        destinationProvince:
          extracted.destinationProvince ?? existingDetails?.destinationProvince,
        serviceType: extracted.serviceType ?? existingDetails?.serviceType,
        cashOnDelivery:
          extracted.cashOnDelivery ?? existingDetails?.cashOnDelivery,
        declaredValue:
          extracted.declaredValue ?? existingDetails?.declaredValue,
        insurance: extracted.insurance ?? existingDetails?.insurance,
      };
    } catch (error) {
      logger.warn(
        "⚠️ [Supervisor] Errore estrazione LLM, uso logica base:",
        error
      );
    }
  }
 ...
 */
function hasEnoughDataForPricing(
  details?: AgentState["shipment_details"]
): boolean {
...
export async function supervisor(
  ...
  try {
    ...
    if (
      ...
      logger.log(
        "📦 [Supervisor] Conferma booking rilevata, verifico preflight..."
      );

      // Verifica pre-flight
      const selectedOption = state.pricing_options[0]; // TODO: permettere selezione
      const idempotencyKey =
        state.shipmentId || `booking-${state.userId}-${Date.now()}`;
      ...
      if (preflight.passed) {
        ...
        try {
          ...
          if (!creditCheck.sufficient) {
            ...
            return {
              clarification_request:
                formatInsufficientCreditMessage(creditCheck),
              next_step: "END",
          ...
          logger.error(
            `❌ [Supervisor] Errore verifica credito: ${errorMessage}`
          );
          // In caso di errore, procedi comunque (booking_worker farà il check)
          logger.warn(
            "⚠️ [Supervisor] Procedo comunque, booking_worker farà il check"
          );
          return {
            next_step: "booking_worker",
            processingStatus: "calculating",
        ...
        logger.log(
          "⚠️ [Supervisor] Preflight fallito, mancano:",
          preflight.missing
        );
        return {
    ...
    if (state.pricing_options && state.pricing_options.length > 0) {
      ...
      if (
        confidenceScore >=
          autoProceedConfig.AUTO_PROCEED_CONFIDENCE_THRESHOLD &&
        !hasValidationErrors
      ) {
        logger.log(
      ...
      if (
        confidenceScore >=
          autoProceedConfig.SUGGEST_PROCEED_CONFIDENCE_THRESHOLD &&
        confidenceScore < autoProceedConfig.AUTO_PROCEED_CONFIDENCE_THRESHOLD &&
        !hasValidationErrors
      ) {
        logger.log(
          `💡 [Supervisor] Suggerimento procedura (confidence: ${confidenceScore}%)`
        );
        return {
          next_step: "END",
          processingStatus: "complete",
      ...
      logger.log(
        "✅ [Supervisor] Preventivi già calcolati, attendo conferma utente"
      );
      return {
        next_step: "END",
        processingStatus: "complete",
    ...
    if (detectMentorIntent(messageText)) {
      logger.log(
        "🎓 [Supervisor] Intent mentor rilevato, routing a mentor_worker"
      );
      return {
    ...
    if (detectExplainIntent(messageText)) {
      logger.log(
        "📚 [Supervisor] Intent explain rilevato, routing a explain_worker"
      );
      return {
    ...
    if (detectDebugIntent(messageText)) {
      logger.log(
        "🐛 [Supervisor] Intent debug rilevato, routing a debug_worker"
      );
      return {
    ...
    if (detectPriceListIntent(messageText)) {
      logger.log(
        "🏷️ [Supervisor] Intent listini rilevato, routing a price_list_worker"
      );
      return {
    ...
    if (hasEnoughData) {
      ...
      if (!shipmentDetails?.weight || shipmentDetails.weight <= 0)
        missing.push("peso");
      if (
        !shipmentDetails?.destinationZip ||
        shipmentDetails.destinationZip.length !== 5
      )
        missing.push("CAP destinazione");
      if (
        !shipmentDetails?.destinationProvince ||
        shipmentDetails.destinationProvince.length !== 2
      )
        missing.push("provincia destinazione");

      logger.log(
        `⚠️ [Supervisor] Dati insufficienti, mancano: ${missing.join(", ")}`
      );
...
export function decideNextStep(input: DecisionInput): SupervisorDecision {
  ...
  if (
    input.hasPricingOptions &&
    input.hasBookingConfirmation &&
