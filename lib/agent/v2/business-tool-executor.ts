...
// Whitelist campi sicuri da sessionState (mai propagare userId/userEmail dal client)
const SESSION_STATE_SAFE_KEYS = new Set([
  'shipmentDraft',
...
function pickSafeSessionState(
  sessionState: Record<string, unknown> | undefined
): Record<string, unknown> {
  if (!sessionState) return {};
  const safe: Record<string, unknown> = {};
  for (const key of SESSION_STATE_SAFE_KEYS) {
    if (key in sessionState) safe[key] = sessionState[key];
  }
  return safe;
}
...
function createBaseAgentState(input: AnneOrchestratorInput): AgentState {
  return {
    ...
    ...pickSafeSessionState(input.sessionState),
  } as AgentState;
}
...
async function executeCustomCapabilityTool(
  ...
  try {
    return await executeCustomCapabilityToolInner(call, context);
  } catch (err) {
    const errMsg = err instanceof Error ? err.message : 'unknown';
    defaultLogger.log(`[ToolExecutor] Errore tool ${call.name}: ${errMsg}`);
    return {
      success: false,
      error: "Si e verificato un errore durante l'esecuzione. Riprova.",
    };
  }
}

async function executeCustomCapabilityToolInner(
  call: ToolCall,
  context: ToolExecutionContext
): Promise<ToolExecutionResult> {
  const { input } = context;
