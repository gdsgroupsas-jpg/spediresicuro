...
test.describe('Gestione Listini Master (Superadmin)', () => {
  ...
  test('1. Pagina carica correttamente per superadmin', async ({ page }) => {
    ...
    const hasRealError = await page
      .locator(
        'text=/500.*error|error.*500|Application error|Internal Server Error|Errore Supabase|Errore 500/i'
      )
      .count();
    ...
    if (hasAccessDenied) {
      console.log('⚠️ Accesso negato - test user non è superadmin (comportamento corretto)');
    } else if (hasHeading) {
  ...
  test('2. Verifica UI componenti principali', async ({ page }) => {
    ...
    if (url.includes('/login')) {
      console.log('⚠️ Skip: Redirected to login - test mode bypass not configured for this route');
      test.skip();
      return;
    }
    ...
    const hasRefreshButton = (await page.locator('button:has-text("Aggiorna")').count()) > 0;
  ...
  test('4. Verifica pulsanti azione nella tabella', async ({ page }) => {
    ...
    const hasCloneButton = (await page.locator('button[title="Clona listino"]').count()) > 0;
    const hasAssignButton = (await page.locator('button[title="Assegna a utente"]').count()) > 0;
    const hasViewButton =
      (await page.locator('button[title="Visualizza assegnazioni"]').count()) > 0;
  ...
  test('5. Test apertura dialog clone', async ({ page }) => {
    ...
    if ((await cloneButton.count()) > 0) {
      ...
      if (dialogVisible) {
        ...
        const hasTitle = (await dialog.locator('text=/Clona Listino/i').count()) > 0;
        const hasNameInput = (await dialog.locator('input#cloneName').count()) > 0;
      ...
      console.log('ℹ️ Nessun pulsante clone disponibile (nessun listino master)');
    }
  });
  ...
  test('6. Test apertura dialog assegnazione', async ({ page }) => {
    ...
    const assignButton = page.locator('button[title="Assegna a utente"]').first();
    if ((await assignButton.count()) > 0) {
      ...
      if (dialogVisible) {
        // Verifica contenuto dialog
        const hasTitle = (await dialog.locator('text=/Assegna Listino/i').count()) > 0;
        const hasUserSelect = (await dialog.locator('select').count()) > 0;
      ...
      console.log('ℹ️ Nessun pulsante assign disponibile (nessun listino master)');
    }
  });
});
