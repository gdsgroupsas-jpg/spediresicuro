...
describe('Encryption Fail-Closed Security', () => {
  ...
  describe('Comportamento in Produzione', () => {
      ...
      function encryptCredential(plaintext: string): string {
        ...
        if (!encryptionKey) {
          if (nodeEnv === 'production') {
            throw new Error('CRITICAL: ENCRYPTION_KEY must be configured in production');
          }
    ...
    it('dovrebbe documentare comportamento fail-closed', () => {
      ...
      expect(securityBehavior.production.withoutEncryptionKey).toBe('THROW_ERROR');
      expect(securityBehavior.development.withoutEncryptionKey).toBe('WARN_AND_USE_PLAINTEXT');
    });
  });
  ...
  describe('isEncrypted() Detection', () => {
    it('dovrebbe rilevare dati criptati', () => {
      ...
      expect(isEncrypted('SGVsbG8gV29ybGQgdGhpcyBpcyBhIHRlc3Qgc3RyaW5n')).toBe(true);
...
describe("Variabili d'Ambiente Encryption", () => {
  ...
  it('dovrebbe validare formato ENCRYPTION_KEY', () => {
    ...
    // Chiave non hex
    expect(validateEncryptionKey('not-a-hex-key!@#$%^&*()'.repeat(3)).valid).toBe(false);
