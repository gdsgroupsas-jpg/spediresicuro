...
describe('Multi-Account Security - Isolamento Configurazioni', () => {
  ...
  describe('Isolamento Multi-Tenant', () => {
    ...
    it('dovrebbe isolare configurazioni tra tenant diversi', () => {
      ...
      expect(tenantAConfigs.map((c) => c.id)).not.toContain('tenant-b-config-1');
      ...
      expect(tenantBConfigs.map((c) => c.id)).not.toContain('tenant-a-config-1');
    ...
    it('dovrebbe prevenire cross-tenant data leakage nei listini', () => {
      ...
      const tenantALists = getPriceListsForUser(allPriceLists, 'tenant-a', false, true);
