...
describe('PRICING_MATRIX - Zone Configuration', () => {
  ...
  it('should have Italy zones before Europe zones', () => {
    const italyZones = PRICING_MATRIX.ZONES.filter((z) => z.code.startsWith('IT-'));
    const europeZones = PRICING_MATRIX.ZONES.filter((z) => z.code.startsWith('EU-'));
...
describe('PRICING_MATRIX - Weight Brackets', () => {
  it('should have standard weight brackets (9 values)', () => {
    expect(PRICING_MATRIX.WEIGHT_BRACKETS_STANDARD).toEqual([2, 5, 10, 20, 30, 50, 70, 100, 105]);
  });
...
describe('getZonesForMode()', () => {
  ...
  it('italy-only mode: should be alias for balanced', () => {
    ...
    expect(italyOnlyZones.map((z) => z.code)).toEqual(balancedZones.map((z) => z.code));
  });
...
describe('estimateSyncCalls()', () => {
  ...
  it('should provide estimated minutes for each mode', () => {
    expect(estimateSyncCalls('fast').estimatedMinutes).toBeGreaterThanOrEqual(0);
