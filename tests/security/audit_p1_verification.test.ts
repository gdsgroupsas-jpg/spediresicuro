...
describe("P1-1 Audit Verification: Ownership Bypass", () => {
  it("SHOUD PREVENT User B from accessing User A config via getSpedisciOnlineCredentials(configId)", async () => {
    ...
    try {
      ...
      const { data: authA, error: errAuthA } =
      ...
      if (errAuthA)
        throw new Error(`Error creating Auth User A: ${errAuthA.message}`);
      ...
      if (puA) {
        ...
        if (errPu)
          throw new Error(
            `Error inserting Public User A: ${errPu.message} (Code: ${errPu.code})`
          );
        userA = puManual;
      ...
      const { data: authB, error: errAuthB } =
        await supabaseAdmin.auth.admin.createUser({
          email: emailB,
          password: "password123",
          email_confirm: true,
      ...
      if (errAuthB)
        throw new Error(`Error creating Auth User B: ${errAuthB.message}`);
      ...
      if (puB) {
        ...
        const { data: puManual, error: errPu } = await supabaseAdmin
          ...
          .single();
        if (errPu)
          throw new Error(`Error inserting Public User B: ${errPu.message}`);
        userB = puManual;
