  ...
  describe("P1-2: DB Lock per Race Condition", () => {
    it("dovrebbe prevenire sync simultanee con DB lock", async () => {
      ...
      const lockErrors = [result1, result2].filter(
        (r) =>
          !r.success &&
          (r.error?.includes("in corso") ||
            r.error?.includes("lock") ||
            r.error?.includes("già"))
      );
    ...
    it("dovrebbe rilasciare lock dopo completamento", async () => {
      ...
      if (lockReleased) {
        ...
        console.warn("⚠️ Lock non rilasciato entro 5 secondi - potrebbe essere normale se la sync è ancora in corso");
      }
    });
    ...
    it("dovrebbe verificare che il lock sia nel database", async () => {
      ...
      expect(lockAfter === null || lockAfter?.status === "in_progress" || lockAfter?.status === "completed").toBe(true);
    });
  });
