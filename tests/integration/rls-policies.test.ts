...
describe("RLS Policies - Isolamento Multi-Tenant", () => {
  ...
  beforeAll(async () => {
    ...
    // try {
    // Crea utenti di test con random suffix per unicità garantita
  ...
  describe("SELECT Policy - Isolamento", () => {
    it("Reseller A dovrebbe vedere SOLO la propria config", async () => {
      ...
      expect(canSeeConfig(resellerAId, false, resellerAId, false)).toBe(true); // Propria config
      expect(canSeeConfig(null, true, resellerAId, false)).toBe(true); // Default
      expect(canSeeConfig(resellerBId, false, resellerAId, false)).toBe(false); // Config Reseller B
    ...
    it("Admin dovrebbe vedere TUTTE le config", async () => {
      ...
      expect(canSeeConfig(resellerAId, false, adminId, true)).toBe(true);
      expect(canSeeConfig(resellerBId, false, adminId, true)).toBe(true);
      expect(canSeeConfig(byocAId, false, adminId, true)).toBe(true);
    ...
    it("Config default dovrebbe essere visibile a tutti", async () => {
      ...
      expect(canSeeConfig(null, true, resellerAId, false)).toBe(true);
      expect(canSeeConfig(null, true, resellerBId, false)).toBe(true);
      expect(canSeeConfig(null, true, byocAId, false)).toBe(true);
    });
  });
  ...
  describe("UPDATE Policy", () => {
    it("Reseller può aggiornare SOLO le proprie config", () => {
      ...
      expect(canUpdateConfig(resellerAId, resellerAId, false)).toBe(true);
  ...
  describe("DELETE Policy", () => {
    it("Reseller può eliminare SOLO le proprie config", () => {
      ...
      expect(canDeleteConfig(resellerAId, resellerAId, false)).toBe(true);

      // Reseller A NON può eliminare config di Reseller B
      expect(canDeleteConfig(resellerBId, resellerAId, false)).toBe(false);
    });
  });
});
