...
describe('OCR Vision Integration Tests', () => {
  ...
  describe.skipIf(skipCondition)('extractData with real Gemini Vision', () => {
    ...
    for (const filename of fixtureNames) {
      ...
      it(`[${expectation.difficulty.toUpperCase()}] ${filename}: ${expectation.description}`, async () => {
        ...
        try {
          ...
          const mapped = mapVisionOutputToShipmentDraft(result.shipmentData as Record<string, unknown>);
          ...
          metrics.errors.push(`${filename}: ${error instanceof Error ? error.message : 'unknown error'}`);
    ...
    it('should meet acceptance criteria', () => {
      ...
      console.log(`   Accuracy: ${(accuracy * 100).toFixed(1)}% (target: >= ${fixturesFile.acceptanceCriteria.minFieldAccuracy * 100}%)`);
      console.log(`   Clarification rate: ${(clarificationRate * 100).toFixed(1)}% (target: <= ${fixturesFile.acceptanceCriteria.maxClarificationRate * 100}%)`);
 ...
 */
function getNestedField(mapped: ReturnType<typeof mapVisionOutputToShipmentDraft>, field: string): string | undefined {
