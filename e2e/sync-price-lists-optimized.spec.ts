...
test.describe('Sync Listini Ottimizzati', () => {
  test.beforeEach(async ({ page }) => {
    // Login con account test (usa stesso pattern del test esistente)
    await page.goto('/login');
    
    // Attendi caricamento form
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(1000);
    
    // Compila form login (prova selettori multipli)
    ...
    try {
      ...
      console.log('❌ Login fallito - verifico stato pagina');
      const currentUrl = page.url();
      console.log('📍 URL corrente:', currentUrl);
      throw new Error(`Login fallito. URL: ${currentUrl}`);
    }
  ...
  test('1. Verifica listini esistenti (compatibilità)', async ({ page }) => {
    ...
    if (!finalUrl.includes('listini-fornitore')) {
      throw new Error(`Non siamo sulla pagina listini-fornitore. URL: ${finalUrl}`);
    }
    ...
    const hasEmptyState = await page.locator('text=/Nessun listino|Sincronizza|Crea Listino/i').count() > 0;
  ...
  test('2. Test cache intelligente', async ({ page }) => {
    ...
    const syncButton = page.locator('text=/sincronizza.*listini/i').or(
      page.locator('text=/sync.*listini/i')
    ).or(
      page.locator('[href*="sync"], [href*="listini"]')
    );
    ...
    if (await syncButton.count() === 0) {
      ...
      const openSyncButton = page.locator('button:has-text("Sincronizza"), button:has-text("Sync")');
    ...
    const syncDialog = page.locator('[role="dialog"]').or(
    ...
    if (await syncDialog.count() > 0) {
      ...
      const overwriteCheckbox = syncDialog.locator('input[type="checkbox"][name*="overwrite"], input[type="checkbox"]:near(text=/sovrascrivi/i)');
      ...
      const syncButton = syncDialog.locator('button:has-text("Sincronizza"), button:has-text("Sync")').first();
      if (await syncButton.count() > 0) {
  ...
  test('3. Test configurazioni manuali', async ({ page }) => {
    ...
    const configureButton = page.locator('button[title="Configura"]').or(
      page.locator('button').filter({ has: page.locator('svg.lucide-settings') })
    ).first();
    ...
    if (buttonCount > 0) {
      ...
      await Promise.all([
        page.waitForSelector('[role="dialog"], [data-state="open"]', { timeout: 10000 }).catch(() => null),
      ...
      let dialog = page.locator('[role="dialog"]').or(
      ...
      if (dialogCount === 0) {
        ...
        // Ricontrolla dialog
        dialog = page.locator('[role="dialog"]').or(
          page.locator('[data-state="open"]')
        ).first();
      ...
      if (dialogCount > 0) {
        ...
        if (await insuranceTab.count() > 0) {
          ...
          const maxValueInput = dialog.locator('input[name*="max"], input[id*="max"], label:has-text("Valore massimo") + input');
        ...
        if (await codTab.count() > 0) {
          ...
          const addRowButton = dialog.locator('button:has-text("Aggiungi"), button:has-text("Add")');
        ...
        if (await servicesTab.count() > 0) {
          ...
          const serviceInputs = dialog.locator('input[placeholder*="servizio"], input[name*="service"]');
        ...
        // Chiudi dialog
        const closeButton = dialog.locator('button:has-text("Annulla"), button:has-text("Chiudi"), [aria-label="Close"]');
        ...
        console.log('⚠️ Dialog non aperto dopo click - potrebbe essere un problema di timing o JavaScript');
      }
    } else {
  ...
  test('4. Test sync con overwrite (bypass cache)', async ({ page }) => {
    ...
    if (await syncButton.count() > 0) {
      ...
      if (await dialog.count() > 0) {
        ...
        const overwriteCheckbox = dialog.locator('input[type="checkbox"][name*="overwrite"], input[type="checkbox"]:near(text=/sovrascrivi/i)');
        if (await overwriteCheckbox.count() > 0) {
        ...
        // Clicca sync
        const confirmSyncButton = dialog.locator('button:has-text("Sincronizza"), button:has-text("Sync")').first();
        if (await confirmSyncButton.count() > 0) {
          // Monitora console per log di bypass cache
          page.on('console', msg => {
            if (msg.text().includes('Cache bypassata') || msg.text().includes('overwriteExisting=true')) {
