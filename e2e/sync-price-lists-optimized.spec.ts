...
test.describe("Sync Listini Ottimizzati", () => {
  ...
  async function goToListiniFornitore(page: any) {
    ...
    if (!url.includes("listini-fornitore") && !url.includes("dati-cliente")) {
      console.log(
        "🔄 Redirect non previsto - provo navigazione manuale via link..."
      );
      ...
      if ((await directLink.count()) > 0) {
        ...
        const resellerButton = page
          .locator("button")
          .filter({ hasText: "Reseller" })
          .first();
        if ((await resellerButton.count()) > 0) {
          await resellerButton.click();
          await page.waitForTimeout(1000);
          const listiniLink = page
            .locator('a[href*="listini-fornitore"]')
            .first();
  ...
  test("1. Verifica listini esistenti (compatibilità)", async ({ page }) => {
    ...
    const syncButton = page
      .getByRole("button", { name: /Sincronizza/i })
      .first();
    const createButton = page
      .getByRole("button", { name: /Crea Listino/i })
      .first();
    ...
    console.log(
      `✅ Pulsanti principali visibili: Sync=${syncVisible}, Create=${createVisible}`
    );
    ...
    const hasEmptyState =
      (await page
        .locator("text=/Nessun listino|Sincronizza|Crea Listino/i")
        .count()) > 0;

    expect(hasTable || hasEmptyState).toBe(true);
    console.log(
      `✅ Contenuto caricato: ${hasTable ? "Tabella" : "Stato vuoto"}`
    );
  ...
  test("2. Test cache intelligente", async ({ page }) => {
    ...
    if ((await syncDialog.count()) > 0) {
      ...
      if ((await syncButton.count()) > 0) {
        ...
        const cacheMessage = page.locator(
          "text=/skip|saltat|cache|già.*sincronizzat/i"
        );
        const successMessage = page.locator(
          "text=/success|completat|creat|aggiornat/i"
        );
      ...
      console.log(
        "⚠️ Dialog sync non trovato - potrebbe essere implementato diversamente"
      );
    }
  });
  ...
  test("3. Test configurazioni manuali", async ({ page }) => {
    ...
    const hasTable = (await page.locator("table").count()) > 0;
    const hasEmptyState =
      (await page.locator("text=/Nessun listino/i").count()) > 0;

    if (!hasTable && !hasEmptyState) {
      console.log(
        "⚠️ Tabella e stato vuoto non trovati - attendo caricamento..."
      );
      await page.waitForTimeout(5000);
    }
    ...
    if (buttonCount > 0) {
      ...
      if (dialogCount === 0) {
        ...
        if (dialogCount === 0 && errors.length > 0) {
          throw new Error(
            `Errore JavaScript durante apertura dialog: ${errors.join(", ")}`
          );
        }
        ...
        if (dialogCount === 0) {
          console.log(
            "⚠️ Dialog non trovato - verifico se pulsante è ancora cliccabile"
          );
      ...
      if (dialogCount > 0) {
        ...
        const insuranceTab = dialog.locator(
          '[role="tab"]:has-text("Assicurazione")'
        );
  ...
  test("5. Verifica performance sync (parallelizzazione)", async ({ page }) => {
    ...
    if ((await syncButton.count()) > 0) {
      ...
      if ((await dialog.count()) > 0) {
        ...
        const modeSelect = dialog.locator(
          'select[name*="mode"], select[id*="mode"]'
        );
        ...
        if ((await confirmSyncButton.count()) > 0) {
          ...
          if (batchLogs.length > 0) {
            console.log(
              `✅ Parallelizzazione attiva: ${batchLogs.length} batch rilevati`
            );
