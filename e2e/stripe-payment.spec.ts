...
test.describe("Stripe Payment Flow", () => {
  ...
  test("Ricarica wallet con Stripe - Flusso completo", async ({ page }) => {
    ...
    if (page.url().includes("/login")) {
      ...
      console.log(
        "⚠️ Redirected to login - skipping test (auth mock not working in this environment)"
      );
      test.skip();
      return;
    }
    ...
    const rechargeButton = page
      .locator(
        'button:has-text("Ricarica Wallet"), button:has-text("Ricarica")'
      )
      .first();

    // Check if button exists, otherwise skip gracefully
    const buttonVisible = await rechargeButton
    ...
    if (!buttonVisible) {
      ...
      console.log(
        "⚠️ Ricarica button not found - page may not have loaded correctly"
      );
      test.skip();
      return;
    }
    ...
    const cardTab = page
      .locator('button:has-text("Carta"), button:has-text("Stripe")')
      .first();
    ...
    const payButton = page
      .locator('button:has-text("Paga"), button:has-text("Procedi")')
      .first();
