...
2. **Permettere BYOC di creare listini fornitore:**
   ...
   if (!isAdmin && !isReseller && !isBYOC) {
     return { success: false, error: 'Solo admin, reseller e BYOC possono creare listini' }
   }
   ```
...
if (isBYOC && data.list_type !== 'supplier') {
  return { success: false, error: 'BYOC può creare solo listini fornitore (list_type = supplier)' }
}
```
...
2. **Validare che BYOC non possa cambiare `list_type`:**
   ...
   if (isBYOC && data.list_type && data.list_type !== 'supplier') {
     return { success: false, error: 'BYOC può modificare solo listini fornitore' }
   }
   ```
...
3. **Validare che Reseller non possa cambiare `list_type` a 'global':**
   ...
   if (isReseller && data.list_type === 'global') {
     return { success: false, error: 'Reseller non può creare listini globali' }
   }
   ```
...
1. **Aggiungere verifica `assigned_to_user_id`:**
   ...
   if (!isAdmin && !isOwner && !isAssignedOwner) {
     return { success: false, error: 'Non hai i permessi per eliminare questo listino' }
   }
   ```
...
2. **Validare che BYOC possa eliminare solo listini fornitore:**
   ...
   if (isBYOC && existingPriceList.list_type !== 'supplier') {
     return { success: false, error: 'BYOC può eliminare solo listini fornitore' }
   }
   ```
...
export async function createSupplierPriceListAction(
  data: Omit<CreatePriceListInput, 'list_type' | 'is_global'> & { courier_id: string }
): Promise<{
  success: boolean
  priceList?: any
  error?: string
  ...
  try {
    ...
    if (!isReseller && !isBYOC) {
      return { success: false, error: 'Solo Reseller e BYOC possono creare listini fornitore' }
    }
...
export async function listSupplierPriceListsAction(): Promise<{
  ...
  try {
    ...
    if (!isReseller && !isBYOC) {
      return { success: false, error: 'Solo Reseller e BYOC possono vedere listini fornitore' }
    }
