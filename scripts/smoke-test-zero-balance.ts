...
async function createTestUser(): Promise<TestUser> {
  ...
  const { data: user, error } = await supabase
    ...
    .insert({
      ...
      wallet_balance: 0.00, // ⚠️ CRITICAL: Zero balance
      account_type: 'user',
      role: 'user'
    })
    .select()
...
async function verifyWalletBalanceUnchanged(userId: string, expectedBalance: number): Promise<boolean> {
  const { data, error } = await supabase
    .from('users')
  ...
  console.log(`💰 Wallet balance: Expected €${expectedBalance.toFixed(2)}, Actual €${actualBalance.toFixed(2)}`)
...
async function main() {
  ...
  try {
    ...
    if (parseFloat(testUser.wallet_balance.toString()) !== 0.00) {
    ...
    const balanceUnchanged = await verifyWalletBalanceUnchanged(testUser.id, 0.00)
