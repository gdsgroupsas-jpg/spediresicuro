...
async function main() {
  ...
  configs?.forEach((c, i) => {
    console.log(`   ${i + 1}. ${c.provider_id} (ID: ${c.id.substring(0, 8)}..., owner: ${c.owner_user_id?.substring(0, 8) || "null"}, default: ${c.is_default})`);
  });
  ...
  for (const cfg of userConfigs) {
    ...
    const testResult = await testAdapter.getRates({
      ...
      shipFrom: { name: "Test", street1: "Via Roma 1", city: "Roma", state: "RM", postalCode: "00100", country: "IT" },
      shipTo: { name: "Test", street1: "Via Milano 1", city: "Milano", state: "MI", postalCode: "20100", country: "IT" },
      notes: "Test",
    });
  ...
  for (const [carrierCode, rates] of Object.entries(ratesByCarrier)) {
    ...
    console.log(`   Sample: carrierCode=${sample.carrierCode}, contractCode=${sample.contractCode}, price=${sample.total_price}`);
  ...
  if (carrierCodes.length !== existingLists?.length) {
    ...
    const existingCarriers = existingLists?.map(l => {
