...
async function verifyMetadataColumn() {
  ...
  try {
    ...
    const { data, error } = await supabase.from('price_lists').select('metadata').limit(1);
    ...
    if (error) {
      if (error.message.includes('metadata') || error.code === 'PGRST204') {
        ...
        console.error(
          '   - Via Supabase Dashboard: SQL Editor → copia/incolla 059_add_metadata_to_price_lists.sql'
        );
