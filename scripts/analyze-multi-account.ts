...
async function main() {
  ...
  for (const cfg of configs || []) {
    ...
    const actualCarriers = cfgLists.map((l: any) => l.meta.carrier_code?.toLowerCase()).filter(Boolean);
  ...
  const expectedByCarrierPerConfig = new Set(allContracts.map(c => `${c.configId}:${c.carrierCode}`)).size;
