...
async function testCompleteFlow() {
  ...
  try {
    ...
    if (dbErrorAfterCallback) {
      ...
      console.log(
        '  - dati_cliente.datiCompletati:',
        dbUserAfterCallback.dati_cliente?.datiCompletati
      );
    } else {
      console.log('  ❌ Record NON esiste dopo callback');
      console.log(
        '  ⚠️ ATTENZIONE: Record dovrebbe essere creato durante /api/auth/supabase-callback'
      );
    }
  } catch (error: any) {
  ...
  const redirectDecision =
    redirectCheckError || !redirectCheck?.dati_cliente || !redirectCheck.dati_cliente.datiCompletati
