...
async function main() {
  ...
  for (const cfg of configs || []) {
    ...
    console.log(
      `   - credentials_encrypted presente: ${!!cfg.credentials_encrypted}`
    );

    if (cfg.credentials) {
      console.log(
        `   - credentials.api_key presente: ${!!cfg.credentials.api_key}`
      );
      console.log(
        `   - credentials.api_secret presente: ${!!cfg.credentials.api_secret}`
      );
      console.log(
        `   - credentials.base_url: ${
          cfg.credentials.base_url || "non definito"
        }`
      );
    }
    ...
    try {
      const adapter = new SpedisciOnlineAdapter({
        api_key: apiKey,
        api_secret:
          cfg.credentials?.api_secret || (cfg as any).api_secret || "",
        base_url:
          cfg.credentials?.base_url ||
          "https://infinity.spedisci.online/api/v2",
