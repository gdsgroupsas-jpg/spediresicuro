...
async function main() {
  ...
  for (const cfg of configs) {
    ...
    try {
      ...
      if (!result.success || !result.rates) {
        console.log(
          "│    ❌ API Error: " +
            (result.error || "unknown").substring(0, 45).padEnd(48) +
            "│"
        );
        console.log("└" + "─".repeat(68) + "┘");
        continue;
      ...
      console.log(
        "│    ✅ API OK - " + rates.length + " rates trovati".padEnd(53) + "│"
      );
      console.log("├" + "─".repeat(68) + "┤");
      ...
      for (const rate of rates) {
        const carrierCode =
          (rate as any).carrier_code || (rate as any).carrierCode || "unknown";
        const contractCode =
          (rate as any).contract_code || (rate as any).contractCode || "N/A";
      ...
      for (const [carrier, info] of seenCarriers) {
        console.log(
          `│       - ${carrier
            .toUpperCase()
            .padEnd(25)} ${info.contractCode.substring(0, 30)}`.padEnd(68) + "│"
        );
      }
    } catch (e: any) {
      console.log(
        "│    ❌ Errore API: " + e.message.substring(0, 45).padEnd(48) + "│"
      );
    }
  ...
  console.log(
    "\n   Listini attesi (1 per corriere per config):",
    expectedListings
  );
  ...
  if (actualListings < expectedListings) {
    ...
    for (const carrier of allCarriers) {
      const found = lists?.find((l) => {
        ...
        return (
          meta.carrier_code === carrier.carrierCode &&
