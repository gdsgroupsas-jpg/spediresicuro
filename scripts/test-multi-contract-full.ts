  ...
  for (const cfg of configs) {
    ...
    try {
      ...
      if (!result.success || !result.rates) {
        console.log(
          '│    ❌ API Error: ' + (result.error || 'unknown').substring(0, 45).padEnd(48) + '│'
        );
        console.log('└' + '─'.repeat(68) + '┘');
        continue;
      }
  ...
  if (actualListings < expectedListings) {
    ...
    for (const carrier of allCarriers) {
      const found = lists?.find((l) => {
        ...
        return (
          meta.carrier_code === carrier.carrierCode && meta.courier_config_id === carrier.configId
        );
      });

      if (!found) {
        console.log(
          '   ❌ Manca: ' + carrier.carrierCode.toUpperCase() + ' per config ' + carrier.configName
        );
      }
    }
