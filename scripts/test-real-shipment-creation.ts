...
async function main() {
  ...
  try {
    ...
    const { data: recentShipments, error: recentError } = await supabase
      .from('shipments')
      .select('id, tracking_number, shipment_id_external, metadata, label_data, status, created_at, user_id')
      .eq('user_id', user.id)
    ...
    if (!recentShipments || recentShipments.length === 0) {
      ...
      console.log('   Per validare shipment_id_external, crea manualmente una spedizione tramite UI')
    ...
    if (!foundValidShipment || !shipmentToCheck) {
      ...
      console.log(`   Questo indica che shipment_id_external non viene salvato durante la creazione.`)
    } else {
      ...
      if (hasExternalId) {
        ...
        if (shipment.metadata) {
          ...
          if (metadataId && String(metadataId) === String(externalIdValue)) {
            ...
            console.log(`   ⚠️  Valore NON corrisponde al metadata: ${metadataId} vs ${externalIdValue}`)
          }
        }
        ...
        console.log(`   Questo significa che l'increment_id non è stato salvato durante la creazione.`)
