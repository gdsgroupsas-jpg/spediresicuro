  ...
  try {
    ...
    if (fs.existsSync(schemaPath)) {
      ...
      if (schemaContent.includes('createShipmentSchema') && schemaContent.includes('z.object')) {
        ...
        const hasRecipient = schemaContent.includes('recipient:') && schemaContent.includes('z.object')
        const hasPackages = schemaContent.includes('packages:') && schemaContent.includes('z.array')
        
        if (hasSender && hasRecipient && hasPackages) {
          console.log('   ✅ Schema contiene sender, recipient, packages')
        } else {
          checks.push({ name: 'Schema completo', status: 'WARN', message: '⚠️  alcuni campi essenziali mancanti' })
    ...
    if (fs.existsSync(routePath)) {
      ...
      if (routeContent.includes('export async function POST') || routeContent.includes('export function POST')) {
        ...
        if (routeContent.includes('createShipmentSchema')) {
          ...
          checks.push({ name: 'Route schema', status: 'WARN', message: '⚠️  route non usa createShipmentSchema' })
  ...
  if (shipmentsError) {
    ...
    results.push({
      ...
      message: allPass 
        ...
        : `Controlli falliti: ${Object.entries(checks).filter(([_, v]) => !v).map(([k]) => k).join(', ')}`,
