...
async function verifyMigration() {
  ...
  try {
    ...
    if (columnError) {
      ...
      const { data, error } = await supabase.from('price_lists').select('list_type').limit(1);
...
async function verifyTypes() {
  ...
  try {
    ...
    const hasCreateInputType =
      typesContent.includes('list_type') && typesContent.includes('CreatePriceListInput');
    const hasUpdateInputType =
      typesContent.includes('list_type') && typesContent.includes('UpdatePriceListInput');
