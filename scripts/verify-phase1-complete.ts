...
async function verifyMigration() {
  ...
  try {
    ...
    const { data: column, error: columnError } = await supabase.rpc(
      "exec_sql",
      {
        sql_query: `
    ...
    if (columnError) {
      ...
      if (
        error &&
        error.message.includes('column "list_type" does not exist')
      ) {
        console.error(
          "❌ Campo list_type NON trovato nella tabella price_lists"
        );
        return false;
      }
...
async function verifyTypes() {
  ...
  try {
    ...
    const hasPriceListType =
      typesContent.includes("list_type") && typesContent.includes("PriceList");
    const hasCreateInputType =
      typesContent.includes("list_type") &&
      typesContent.includes("CreatePriceListInput");
    const hasUpdateInputType =
      typesContent.includes("list_type") &&
      typesContent.includes("UpdatePriceListInput");
...
async function verifyHelperFunction() {
  ...
  try {
    ...
    if (typeof getAvailableCouriersForUser !== "function") {
      console.error(
        "❌ Funzione getAvailableCouriersForUser non è una funzione"
      );
      return false;
    }

    console.log(
      "✅ Funzione getAvailableCouriersForUser() esportata correttamente"
    );

    // Test con userId inesistente (dovrebbe ritornare array vuoto)
    console.log("\n📋 Test con userId inesistente (dovrebbe ritornare [])...");
    const resultEmpty = await getAvailableCouriersForUser(
    ...
    if (configsError) {
      ...
      const contractMapping =
        (configs[0].contract_mapping as Record<string, string>) || {};
      const courierKeys = Object.keys(contractMapping);
      console.log(
        `   Contract mapping keys (corrieri): ${
          courierKeys.length > 0 ? courierKeys.join(", ") : "nessuno"
        }`
      );
      ...
      console.log(
        `✅ Funzione eseguita: trovati ${resultReal.length} corrieri`
      );
      if (resultReal.length > 0) {
        console.log("\n📋 Corrieri trovati:");
        resultReal.forEach((courier, index) => {
          console.log(
            `   ${index + 1}. ${courier.courierName} (ID: ${
              courier.courierId
            }, Provider: ${courier.providerId})`
          );
        });
        ...
        console.log(
          "   Questo può essere normale se le chiavi non corrispondono ai nomi nella tabella couriers"
        );
      ...
      console.log(
        "   (La funzione è comunque corretta, serve solo un utente con configurazioni)"
      );
    }
...
async function verifyDatabaseStructure() {
  ...
  try {
    ...
    if (priceListError) {
      console.error(
        "❌ Errore accesso tabella price_lists:",
    ...
    if (configError) {
      console.error(
        "❌ Errore accesso tabella courier_configs:",
        configError.message
      );
      return false;
    }
...
async function main() {
  ...
  console.log("");
  console.log(
    `Migration 056:        ${results.migration ? "✅ PASS" : "❌ FAIL"}`
  );
  console.log(`Types TypeScript:     ${results.types ? "✅ PASS" : "❌ FAIL"}`);
  console.log(
    `Struttura Database:  ${results.database ? "✅ PASS" : "❌ FAIL"}`
  );
  console.log(
    `Funzione Helper:      ${results.helper ? "✅ PASS" : "❌ FAIL"}`
  );
  console.log("");
