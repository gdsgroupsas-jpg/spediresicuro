...
async function runAudit() {
  ...
  if (err1) {
    ...
    if (anomalies.length > 0) {
      console.error(
        `❌ FAIL: Found ${anomalies.length} duplicate assignments:`,
        anomalies
      );
      hasErrors = true;
    } else {
  ...
  if (err2) {
    ...
    const creatorIds = [
      ...new Set(supplierLists.map((l) => l.created_by).filter((id) => id)),
    ];
      ...
      if (errUsers) {
        ...
        if (violations.length > 0) {
          ...
          violations.forEach((v) => {
            ...
            console.log(
              `   - List: ${v.name} (${v.id}), User: ${u?.email} (${u?.account_type})`
            );
          });
          hasErrors = true;
        } else {
          console.log(
            "✅ PASS: All supplier lists are owned by authorized roles."
          );
        }
      }
    } else {
  ...
  if (err3) {
    ...
    const creatorIds = [
      ...new Set(allLists?.map((l) => l.created_by).filter((id) => id)),
    ];
    ...
    if (errUsers) {
      ...
      const orphans = allLists?.filter(
        (l) => l.created_by && !existingUserIds.has(l.created_by)
      );

      if (orphans && orphans.length > 0) {
        console.error(
          `❌ FAIL: Found ${orphans.length} orphaned price lists (creator not found).`
        );
  ...
  if (err4) {
    ...
    if (masterIds.length > 0) {
      ...
      if (errMasters) {
        ...
        const brokenLinks = derivedLists?.filter(
          (l) => !existingMasters.has(l.master_list_id)
        );

        if (brokenLinks && brokenLinks.length > 0) {
          console.error(
            `❌ FAIL: Found ${brokenLinks.length} broken master list links.`
          );
          hasErrors = true;
        } else {
          console.log(
            "✅ PASS: All derived lists point to valid master lists."
          );
        }
      }
    } else {
