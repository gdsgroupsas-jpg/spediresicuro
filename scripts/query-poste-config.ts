...
async function queryPosteConfig() {
  ...
  try {
    ...
    configs.forEach((config, i) => {
      ...
      console.log('API Key:', config.api_key ? `✅ Presente (${config.api_key.length} caratteri)` : '❌ Mancante');
      console.log('API Secret:', config.api_secret ? `✅ Presente (${config.api_secret.length} caratteri)` : '❌ Mancante');
    ...
    console.log(`   Con CDC configurato: ${configs.filter(c => {
