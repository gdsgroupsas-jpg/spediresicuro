...
if (anthropicKey) {
  ...
  console.log(
    `   Formato: ${anthropicKey.startsWith('sk-ant-') ? '✅ Corretto' : '⚠️ Formato non standard'}`
  );
} else {
...
if (deepseekKey) {
  ...
  console.log(
    `   Formato: ${deepseekKey.startsWith('sk-') ? '✅ Corretto' : '⚠️ Formato non standard'}`
  );
} else {
...
if (googleKey) {
  ...
  console.log(
    `   Formato: ${googleKey.length > 20 ? '✅ Sembra valida' : '⚠️ Formato non standard'}`
  );
} else {
...
providers.forEach((provider) => {
  ...
  console.log(
    `   ${icon} ${provider.name}: ${provider.status ? 'Disponibile' : 'Non disponibile'}`
  );
});

console.log('\n💡 NOTA:');
console.log('   - Locale: Verifica .env.local nella root del progetto');
console.log('   - Produzione: Verifica Vercel Dashboard > Settings > Environment Variables');
console.log('   - Il componente UI mostra lo stato delle API keys in /dashboard/super-admin\n');
...
if (!atLeastOne) {
  ...
  console.log('⚠️  Alcune API keys mancanti. Alcuni provider non saranno disponibili.');
  process.exit(0);
} else {
