...
async function main() {
  ...
  for (const [carrierCode, carrierRates] of Object.entries(ratesByCarrier)) {
    ...
    try {
      ...
      console.log(`   🔍 Ricerca listino: carrier_code=${carrierCode}, configId=${CONFIG_ID.substring(0, 8)}`);
      ...
      if (existingPriceList) {
        console.log(`   📦 Listino esistente: ${existingPriceList.name} (${existingPriceList.id.substring(0, 8)}...)`);
