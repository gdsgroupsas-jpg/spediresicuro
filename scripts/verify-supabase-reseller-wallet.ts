...
async function verifyColumnExists(tableName: string, columnName: string): Promise<boolean> {
  try {
    ...
    const { error: queryError } = await supabaseAdmin.from(tableName).select(columnName).limit(1);
...
async function verifyFunctionExists(functionName: string): Promise<boolean> {
  try {
    ...
    if (error?.message?.includes('does not exist') || error?.message?.includes('non esiste')) {
      return false;
    }
...
async function main() {
  ...
  for (const col of usersColumns) {
    ...
    if (exists) {
      addResult(`Campo users.${col.name}`, '✅', `Esiste (${col.type}) - ${col.description}`);
    } else {
      addResult(`Campo users.${col.name}`, '❌', `MANCANTE - ${col.description}`);
    }
  }
  ...
  if (walletTableExists) {
    ...
    addResult(
      ...
      '❌',
      'MANCANTE - Esegui migration 019_reseller_system_and_wallet.sql'
    );
  }
  ...
  // Test is_super_admin
  try {
    const { data: superAdminTest, error: superAdminError } = await supabaseAdmin.rpc(
      'is_super_admin',
      {
        p_user_id: '00000000-0000-0000-0000-000000000000',
      }
  ...
  try {
    ...
    if (walletTableExists) {
      ...
      if (!transactionsError) {
        ...
        addResult('Transazioni Wallet', '✅', `Trovate ${count || 0} transazioni`);
      }
    }
    ...
    if (!walletError && usersWithWallet) {
      const totalBalance = usersWithWallet.reduce(
        (sum, u) => sum + (parseFloat(u.wallet_balance?.toString() || '0') || 0),
        0
      );
      addResult(
        'Wallet Balance',
      ...
      try {
        ...
        if (!addError && txId) {
          ...
          const expectedBalance =
            (parseFloat(testUser.wallet_balance?.toString() || '0') || 0) + testAmount;
          const actualBalance = parseFloat(updatedUser?.wallet_balance?.toString() || '0') || 0;

          if (Math.abs(actualBalance - expectedBalance) < 0.01) {
            addResult(
              'Test trigger wallet_balance',
              '✅',
              'Balance aggiornato correttamente dal trigger'
            );
          } else {
            addResult(
              'Test trigger wallet_balance',
              '⚠️',
              `Balance potrebbe non essere aggiornato (atteso: ${expectedBalance}, attuale: ${actualBalance})`
            );
          }
          ...
          addResult(
            'Test add_wallet_credit',
            '❌',
            `Errore: ${addError?.message || 'Sconosciuto'}`
          );
        }
      } catch (error: any) {
  ...
  if (errorCount === 0) {
    ...
    console.log('\n📝 Azioni consigliate:');
    console.log(
      '   1. Esegui la migration: supabase/migrations/019_reseller_system_and_wallet.sql'
    );
