...
async function main() {
  ...
  const user = await createSmokeTestUser({ supabase, initialBalance: 50, label: 'db-fail-after-label' })
  ...
  try {
    const result = await createShipmentCore({
      ...
      deps: {
        ...
        overrides: {
          insertShipment: async () => ({ data: null, error: { message: 'FORCED_DB_INSERT_FAILURE' } }),
        },
      },
    })
    ...
    // No shipment row
    const shipments = await fetchRows({ supabase, table: 'shipments', filter: { user_id: user.id }, columns: 'id' })
    ...
    if (deleteRows.length < 1) {
      throw new Error(`Expected DELETE PENDING in compensation_queue, got: ${JSON.stringify(queue)}`)
    }

    console.log('✅ TEST PASSATO: label creata ma DB fallisce → refund + enqueue DELETE se delete fallisce')
  } finally {
