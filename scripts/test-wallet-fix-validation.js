...
async function testDeductWalletCreditIsAtomic() {
  ...
  try {
    ...
    const balanceCorrect = Math.abs((updatedUser?.wallet_balance || 0) - 50.00) < 0.01
    ...
    const txAmountCorrect = txCreated && Math.abs(transactions[0].amount - (-50.00)) < 0.01
  ...
  try {
    ...
    if (insertError) {
      ...
      if (insertError.message.includes('status') || 
          ...
          insertError.code === '42703') { // PostgreSQL: undefined column
        return {
...
async function testNoRaceCondition() {
  ...
  try {
    ...
    const successes = raceResults.filter(r => 
      r.status === 'fulfilled' && !r.value.error
    )
    const failures = raceResults.filter(r => 
    ...
    const expectedBehavior = 
      successes.length === 1 && 
      failures.length === 1 &&
      Math.abs((finalUser?.wallet_balance || 0) - 0.00) < 0.01
