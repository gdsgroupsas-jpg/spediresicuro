...
async function main() {
  ...
  for (const cfg of configs || []) {
    ...
    if (creds.api_key) {
      ...
      if (isCrypted) {
        try {
          ...
          console.log('   API Key (primi 10):', decrypted?.substring(0, 10) + '...');
        } catch (e) {
    ...
    if (contracts.length === 0) {
      ...
      for (const contractCode of contracts) {
        console.log(`      - ${contractCode} → ${contractMapping[contractCode]}`);
      }
    }
  ...
  for (const cfg of configs || []) {
    ...
    try {
      ...
      const carriers = [...new Set(rates.map((r: any) => r.carrier_code || r.carrierCode))];
      const contractCodes = [...new Set(rates.map((r: any) => r.contract_code || r.contractCode))];
